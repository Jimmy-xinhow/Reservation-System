...
export default function ContactPage() {
  return <MarketingShell>
    <PageIntro eyebrow="Start with the real workflow" title="先談你的工作方式，再談要開哪些功能。" description="我們不從套版畫面開始，而是先理解你的品牌、服務目標、資源與顧客入口，再一起決定適合的產品模組與導入順序。" visual={<PageIntroVisual variant="contact" />}><a href="tel:079721612" className="btn min-h-12 bg-[#1f4550] px-5 text-white hover:bg-[#193b43]">撥打導入專線 <span className="text-[#e2b644]">07-9721612#888</span></a><Link href="/product" className="btn min-h-12 border border-[#1f4550]/20 bg-white/70 px-5 text-[#1f4550] hover:bg-white">先看產品能力</Link></PageIntro>

    <section className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 sm:py-24 lg:grid-cols-[.85fr_1.15fr] lg:gap-20 lg:px-10"><div><SectionHeading eyebrow="Implementation conversation" title="一次討論，先把最重要的問題說清楚。" description="如果你正在評估預約／報名系統，建議先帶著以下資訊來，不需要先整理成正式規格書。" /><div className="mt-8 rounded-2xl bg-[#1f4550] p-6 text-white"><p className="text-xs uppercase tracking-[0.2em] text-[#e2b644]">導入專線</p><a href="tel:079721612" className="mt-4 block text-2xl font-bold tracking-wide hover:text-[#e2b644]">07-9721612#888</a><p className="mt-3 text-sm leading-6 text-[#c9dcda]">適合先討論品牌類型、使用人數、顧客入口與目前最卡的流程。</p></div></div><div className="grid gap-3 sm:grid-cols-2">{steps.map(([title, description], index) => <article key={title} className="rounded-2xl border border-[#ddd7ca] bg-white p-5 shadow-[0_8px_30px_rgba(31,69,80,.05)]"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#edf2ef] text-xs font-bold text-[#1f4550]">{String(index + 1).padStart(2, "0")}</span><h2 className="mt-7 font-bold text-[#193b43]">{title}</h2><p className="mt-2 text-sm leading-6 text-[#6d7b76]">{description}</p></article>)}</div></section>
