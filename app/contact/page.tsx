...
import { Callout, FeatureIcon, MarketingShell, ModuleInterface, PageIntro, PageIntroVisual, PhotoBand, SectionHeading, SignalStrip, WorkflowRail } from "@/components/MarketingLayout";
...
export default function ContactPage() {
  return <MarketingShell>
    ...
    <section className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 sm:py-24 lg:grid-cols-[.85fr_1.15fr] lg:gap-20 lg:px-10"><div><SectionHeading eyebrow="Implementation conversation" title="一次討論，先把最重要的問題說清楚。" description="如果你正在評估預約／報名系統，建議先帶著以下資訊來，不需要先整理成正式規格書。" /><div className="mt-8 rounded-2xl bg-[#1f4550] p-6 text-white"><p className="text-xs uppercase tracking-[0.2em] text-[#e2b644]">導入聯絡方式</p><a href="tel:079721612" className="mt-4 block text-2xl font-bold tracking-wide hover:text-[#e2b644]">07-9721612</a><a href="mailto:service@xinhow.com.tw" className="mt-3 block break-all text-sm text-[#dce9e6] hover:text-[#e2b644]">service@xinhow.com.tw</a><a href="https://lin.ee/jnAfCBy" target="_blank" rel="noreferrer" className="mt-4 inline-flex min-h-11 items-center rounded-lg bg-[#e2b644] px-4 text-sm font-semibold text-[#193b43] hover:bg-[#f1ca5b]">官方 LINE · @xinhow</a><p className="mt-3 text-sm leading-6 text-[#c9dcda]">適合先討論品牌類型、使用人數、顧客入口與目前最卡的流程。</p></div></div><div><div className="grid gap-3 sm:grid-cols-2">{steps.map(([title, description], index) => <article key={title} className="rounded-2xl border border-[#ddd7ca] bg-white p-5 shadow-[0_8px_30px_rgba(31,69,80,.05)]"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#edf2ef] text-xs font-bold text-[#1f4550]">{String(index + 1).padStart(2, "0")}</span><h2 className="mt-7 font-bold text-[#193b43]">{title}</h2><p className="mt-2 text-sm leading-6 text-[#6d7b76]">{description}</p></article>)}</div><ModuleInterface kind="entrance" /></div></section>

    <section className="bg-[#193b43] px-5 py-16 text-white sm:px-8 sm:py-20 lg:px-10"><div className="mx-auto max-w-7xl"><div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr] lg:items-end lg:gap-16"><div><p className="eyebrow !text-[#e2b644]">Choose the easiest channel</p><h2 className="text-[clamp(1.75rem,3.2vw,2.75rem)] font-bold leading-[1.12] tracking-[-0.04em]">不用先寫規格書先從一個問題開始</h2><p className="mt-4 max-w-xl text-base leading-7 text-[#c9dcda]">依你的方便選擇電話、Email 或官方 LINE，我們再把品牌、入口與工作流程整理成可執行的導入路徑。</p></div><SignalStrip dark items={[{ label: "電話", value: "07-9721612", detail: "直接討論目前卡點" }, { label: "Email", value: "service@xinhow.com.tw", detail: "適合補充品牌資料" }, { label: "LINE", value: "@xinhow", detail: "官方帳號快速聯絡" }, { label: "操作介面", value: "品牌後台", detail: "先看工作流程" }]} /></div><div className="mt-10"><WorkflowRail dark steps={[{ label: "01", title: "說明現況", detail: "品牌、服務與入口" }, { label: "02", title: "拆解流程", detail: "角色、排程與資料" }, { label: "03", title: "確認範圍", detail: "標準功能與加購" }, { label: "04", title: "規劃導入", detail: "交付、訓練與驗收" }]} /></div></div></section>

    <PhotoBand src="/marketing/contact-onboarding.png" alt="品牌業主與導入顧問討論服務流程" eyebrow="把真實需求帶來" title="從一個現場問題開始規劃導入" description="不需要先寫完整規格書，先把品牌、服務、入口與團隊目前卡住的地方說清楚。" />
