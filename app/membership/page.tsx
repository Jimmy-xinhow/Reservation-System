...
import { safeLocalStorageGet, safeLocalStorageRemoveMatching } from "@/lib/browser-storage";
...
function storedCustomerToken(): string | null {
  ...
  return safeLocalStorageGet(customerTokenKey(), `booking_browser_token:${scope}`, "membership_browser_token");
}
...
export default function MembershipPage() {
  const [recordsPath, setRecordsPath] = useState("/my");
  ...
  const loadPortal = useCallback(async (browserToken?: string, identity?: { name: string; phone: string; birthday: string }) => {
    ...
    try {
      ...
      if (!response.ok || !body?.ok || !body.data) throw new CustomerApiError(body?.error ?? "會員資料查詢失敗", response.status);
      setVerified(true);
      ...
      if (request !== loadRequest.current) return;
      if (browserToken && loadError instanceof CustomerApiError && [401, 403].includes(loadError.status)) {
        setToken(null); setVerified(false); setMemberships([]); setPlans([]);
        const key = customerTokenKey();
      ...
      setError(loadError instanceof TypeError ? "連線失敗，請確認網路後重試" : loadError instanceof Error ? loadError.message : "會員資料查詢失敗");
    } finally {
      if (request === loadRequest.current) { lookupBusy.current = false; setLoading(false); }
    }
  }, []);

  useEffect(() => {
    setRecordsPath(brandedPath("/my"));
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (lookupBusy.current || purchaseLock.current) return;
  ...
  function clearIdentity() {
    ...
    if (token) {
      const key = customerTokenKey();
      safeLocalStorageRemoveMatching([key, key.replace("customer_browser_token:", "booking_browser_token:"), "membership_browser_token"], token);
    }
    setToken(null); setMemberships([]); setPlans([]); setError(null);
    setName(""); setPhone(""); setBirthday("");
  }

  async function purchase(plan: Plan) {
    if (purchaseLock.current || lookupBusy.current) return;
    if (!token || !verified) {
  ...
  return (
    ...
    <div className="flex flex-col gap-6">
      <header className="flex items-start justify-between gap-3">
        ...
        <Link href={recordsPath} className="shrink-0 text-sm text-brand-700">我的紀錄</Link>
      </header>

      {token ? <div className="card flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium text-slate-900">{verified ? "已完成顧客驗證" : "尚待確認顧客身分"}</p><p className="mt-1 text-sm text-slate-500">之後可直接查看會員資料與購買套票。</p></div><button type="button" className="btn btn-secondary w-full sm:w-auto" disabled={buying !== null} onClick={clearIdentity}>更換顧客</button></div> : <form onSubmit={submit} className="card grid gap-4 p-5 sm:grid-cols-3"><label className="text-sm"><span className="label">姓名</span><input className="input" value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" required /></label><label className="text-sm"><span className="label">電話</span><input className="input" value={phone} onChange={(event) => setPhone(event.target.value)} inputMode="tel" autoComplete="tel" required /></label><label className="text-sm"><span className="label">出生年月日</span><input type="date" className="input" value={birthday} onChange={(event) => setBirthday(event.target.value)} required /></label><button className="btn btn-primary sm:col-span-3" disabled={loading}>{loading ? "查詢中…" : "查詢會員資料"}</button></form>}

      {token && <button type="button" disabled={loading || buying !== null} onClick={() => void loadPortal(token)} className="btn btn-secondary">重新查詢會員資料</button>}
