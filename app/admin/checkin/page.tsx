...
export default function CheckinPage() {
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header"><div><div className="eyebrow">活動現場作業</div><h1 className="admin-page-title">報名報到</h1><p className="admin-page-description">可掃描 QR（二維條碼）、輸入報到憑證，或用姓名／電話／報名編號搜尋後手動報到。</p></div></div>

      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.9fr)]">
        <section className="admin-section space-y-4 p-5">
          <div className="overflow-hidden rounded bg-slate-950"><video ref={videoRef} className={`aspect-video w-full object-cover ${scanning ? "block" : "hidden"}`} playsInline muted /><div className={`${scanning ? "hidden" : "flex"} aspect-video items-center justify-center px-6 text-center text-sm text-white/70`}>開啟相機後，將報到二維條碼放入框內掃描</div></div>
        ...
        <section className="admin-section space-y-4 p-5">
          ...
          <div className="divide-y divide-slate-200 border-y border-slate-200">{searchRows.length === 0 ? <p className="bg-slate-50 px-4 py-6 text-center text-sm text-slate-400">輸入關鍵字後顯示可報到名單</p> : searchRows.map((row) => { const event = one(row.events); const session = one(row.event_sessions); const alreadyChecked = row.status === "attended"; return <div key={row.id} className="p-4"><div className="flex items-start justify-between gap-3"><div><div className="font-semibold text-slate-900">{row.name}</div><div className="mt-1 text-xs text-slate-500">{row.registration_no} · {row.phone}</div></div><span className={`badge ${alreadyChecked ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-700"}`}>{alreadyChecked ? "已報到" : "可報到"}</span></div><div className="mt-2 text-sm text-slate-600">{event?.title ?? "活動"}{session ? ` · ${session.name}` : ""}</div><div className="mt-3 flex justify-end">{alreadyChecked ? <span className="text-xs text-slate-400">已完成報到</span> : <button type="button" disabled={loading} onClick={() => void checkinById(row.id)} className="btn btn-primary px-3 py-1.5 text-xs">確認報到</button>}</div></div>; })}</div>
        </section>
      </div>

      <section className="admin-section space-y-3 p-5"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="font-semibold text-slate-900">今日報到工作台</h2><p className="mt-1 text-xs text-slate-500">每 10 秒更新，供櫃檯掌握當日活動報到進度。</p></div><span className="badge bg-slate-100 text-slate-600">{liveRows.filter((row) => row.status === "attended").length} / {liveRows.length} 已報到</span></div>{liveRows.length === 0 ? <p className="border-y border-slate-200 bg-slate-50 p-5 text-center text-sm text-slate-400">今日沒有可報到的活動名單。</p> : <div className="divide-y divide-slate-200 border-y border-slate-200 md:grid md:grid-cols-2 md:divide-x md:divide-y-0 lg:grid-cols-3">{liveRows.map((row) => { const event = one(row.events); const session = one(row.event_sessions); return <div key={row.id} className="p-3"><div className="flex items-center justify-between gap-2"><span className="font-medium text-slate-800">{row.name}</span><span className={`badge ${row.status === "attended" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{row.status === "attended" ? "已報到" : "待報到"}</span></div><p className="mt-1 text-xs text-slate-500">{row.registration_no} · {event?.title ?? "活動"}</p><p className="mt-1 text-xs text-slate-400">{session?.name ?? "場次"} · {session?.start_at ? new Date(session.start_at).toLocaleTimeString("zh-TW", { hour: "2-digit", minute: "2-digit" }) : ""}</p></div>; })}</div>}</section>

      {error && <p className="border border-red-200 border-l-[3px] border-l-red-600 bg-red-50 p-4 text-sm text-red-700">{error}</p>}
