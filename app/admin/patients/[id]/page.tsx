...
import { updatePatientAction, setPatientBlockAction } from "../../actions";
...
export default async function PatientDetailPage({
  ...
  const { data } = await supabase
    .from("patients")
    .select("id, name, phone, note, tags, birthday, gender, email, marketing_opt_in, blocked_until")
    .eq("id", id)
    ...
    <div className="space-y-5">
      ...
      <form action={updatePatientAction} className="card space-y-4 p-5">
        ...
        <label className="block text-sm">
          ...
          <textarea name="note" rows={3} defaultValue={p.note ?? ""} className="input" />
        </label>
