...
export default function FollowupComposer({ action, fixedPatient, className = "", compact = false }: { action: ServerAction; fixedPatient?: PatientOption; className?: string; compact?: boolean }) {
    ...
    <form action={action} className={className}>
      ...
      <div className={`line-live-editor-layout ${compact ? "compact" : ""}`}>
        <div className="line-live-editor-fields compact">
          {fixedPatient ? <div><h2 className="font-semibold text-slate-900">安排指定日期回訪</h2><p className="mt-1 text-xs text-slate-500">顧客：{fixedPatient.name} · {fixedPatient.phone}</p></div> : <PatientPicker />}
