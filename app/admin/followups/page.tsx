...
const CHANNEL: Record<string, string> = { line: "LINE", email: "Email", phone: "電話", manual: "人工處理" };
...
function StateForm({ id, status, label, danger = false }: { id: string; status: string; label: string; danger?: boolean }) { return <form action={setScheduledFollowupStatusAction}><input type="hidden" name="id" value={id} /><input type="hidden" name="status" value={status} /><SubmitButton className={`admin-inline-action ${danger ? "text-red-700" : ""}`}>{label}</SubmitButton></form>; }
