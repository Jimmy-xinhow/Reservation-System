...
export default async function PlatformPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ data: brands, error: brandError }, { data: entitlements, error: entitlementError }, { data: members, error: memberError }] = await Promise.all([
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3"><Metric label="品牌總數" value={brandRows.length} /><Metric label="啟用品牌" value={brandRows.filter((brand) => brand.active).length} /><Metric label="品牌成員" value={memberRows.length} /></div>
      ...
      <section className="space-y-4"><div><h2 className="text-lg font-bold text-slate-900">品牌與方案</h2><p className="mt-1 text-sm text-slate-500">停用採 soft-delete，不刪除既有預約、報名、CRM 或稽核資料。</p></div>{brandRows.length === 0 ? <div className="card p-6 text-sm text-slate-500">尚未建立品牌。</div> : <div className="space-y-4">{brandRows.map((brand) => { const entitlement = entitlementByBrand.get(brand.id); const flags = entitlement?.feature_flags ?? {}; return <article key={brand.id} className="card space-y-5 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h3 className="font-semibold text-slate-900">{brand.name}</h3><span className={`badge ${brand.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{brand.active ? "啟用" : "已停用"}</span></div><p className="mt-1 text-xs text-slate-400">/{brand.slug ?? "未設定代號"} · {membersByBrand.get(brand.id) ?? 0} 位成員</p></div><form action={setPlatformBrandActiveAction}><input type="hidden" name="clinic_id" value={brand.id} /><input type="hidden" name="active" value={brand.active ? "false" : "true"} /><SubmitButton className="btn btn-secondary px-3 py-1.5 text-xs">{brand.active ? "停用品牌" : "重新啟用"}</SubmitButton></form></div><form action={updatePlatformEntitlementAction} className="space-y-4 border-t border-slate-100 pt-4"><input type="hidden" name="clinic_id" value={brand.id} /><div className="grid gap-4 sm:grid-cols-[180px_1fr]"><label className="text-sm"><span className="label">方案</span><select name="plan_code" className="input" defaultValue={entitlement?.plan_code ?? "standard"}><option value="standard">Standard</option><option value="professional">Professional</option><option value="enterprise">Enterprise</option></select></label><label className="text-sm"><span className="label">平台備註</span><input name="note" className="input" defaultValue={entitlement?.note ?? ""} placeholder="合約、客製範圍或交付備註" /></label></div><div><p className="label">另行報價加購能力</p><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{PLATFORM_ADD_ONS.map(({ key, label }) => <label key={key} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"><input type="checkbox" name={key} defaultChecked={flags[key] === true} className="h-4 w-4 accent-brand-600" />{label}</label>)}</div></div><SubmitButton className="btn btn-primary px-4 py-2 text-sm">儲存方案與加購權限</SubmitButton></form></article>; })}</div>}</section>
    </div>
  );
}
