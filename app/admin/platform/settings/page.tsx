...
export default async function PlatformSettingsPage() {
  ...
  return (
    <div className="platform-workbench">
      ...
      <section className="platform-panel space-y-4 p-5 sm:p-6">
        ...
        <div className="divide-y divide-slate-200 border-y border-slate-200">
          {governanceChecks.map((check) => (
            <div key={check.label} className="grid gap-1 py-3 sm:grid-cols-[12rem_minmax(0,1fr)] sm:items-start sm:gap-4">
              <span className="text-sm font-medium text-slate-800">{check.label}</span>
              <span className={`text-xs leading-5 sm:text-right ${check.tone === "good" ? "text-emerald-700" : "text-amber-700"}`}>{check.value}</span>
            </div>
          ))}
        </div>
      ...
      <section className="platform-panel p-5 sm:p-6">
        <p className="text-sm font-semibold text-slate-950">品牌設定在哪裡？</p>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">請先在品牌管理頁建立或查看品牌，再回到品牌後台處理服務、時段、LINE、訊息、顧客回訪與報表。系統管理控制台不代替品牌日常操作。</p>
