...
export default async function SystemPeoplePage() {
  ...
  return (
    <div className="space-y-8">
      <header>
        <p className="eyebrow">System access control</p>
      ...
      <section className="grid gap-4 lg:grid-cols-2" aria-label="管理者身分差異">
        <IdentityCard title="系統管理者" scope="跨品牌系統層級" description="管理品牌租戶、系統人員、營運健康、跨品牌報表、稽核及系統設定。" />
      ...
      <section className="card space-y-5 border-indigo-100 bg-indigo-50/40 p-5 sm:p-6">
        <div><p className="eyebrow">Add system member</p><h2 className="mt-1 text-lg font-bold text-slate-900">新增系統人員</h2><p className="mt-1 text-sm leading-6 text-slate-500">選擇「系統管理者」即取得完整權限；選擇「系統員工」時，再勾選實際需要的工作權限。</p></div>
          ...
          {members.length === 0 ? <p className="px-5 py-8 text-center text-sm text-slate-500">尚未設定系統人員。</p> : members.map((member) => {
            ...
            return (
              <article key={member.user_id} className="space-y-4 px-5 py-5 sm:px-6">
                <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-medium text-slate-900">{email}{isSelf && <span className="ml-2 badge bg-indigo-50 text-indigo-700">目前登入</span>}</p><p className="mt-1 text-xs text-slate-400">加入於 {formatDate(member.created_at)} · ID {member.user_id}</p></div><div className="flex items-center gap-2"><span className={`badge ${member.access_type === "system_admin" ? "bg-indigo-50 text-indigo-700" : "bg-slate-100 text-slate-600"}`}>{platformAccessLabel(member.access_type)}</span><span className={`badge ${member.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{member.active ? "啟用中" : "已停用"}</span></div></div>
