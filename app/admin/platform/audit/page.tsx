...
export default async function PlatformAuditPage() {
  ...
  return <div className="space-y-8"><header><p className="eyebrow">System governance</p><h1 className="mt-1 text-2xl font-bold text-slate-950">系統稽核</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">集中查看跨品牌的預約、報名與付款狀態異動，協助系統管理者判斷問題是否來自單一品牌、通知流程或金流回呼。</p></header><section className="card overflow-hidden"><div className="flex flex-wrap items-end justify-between gap-3 border-b border-slate-100 px-5 py-4 sm:px-6"><div><h2 className="font-semibold text-slate-900">最近 200 筆狀態異動</h2><p className="mt-1 text-sm text-slate-500">事件依時間排序；操作者 ID 僅供系統稽核使用。</p></div><span className="badge bg-indigo-50 text-indigo-700">跨品牌</span></div><div className="overflow-x-auto"><table className="tbl"><thead><tr><th>時間</th><th>品牌</th><th>類型</th><th>狀態變更</th><th>來源</th><th>操作者</th><th>備註</th></tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={7} className="py-10 text-center text-sm text-slate-400">尚無跨品牌稽核紀錄。</td></tr> : rows.map((row) => <tr key={`${row.kind}-${row.id}`}><td className="whitespace-nowrap text-xs text-slate-500">{formatDateTime(row.created_at)}</td><td className="font-medium text-slate-800">{brandsById.get(row.clinic_id) ?? "未知品牌"}</td><td><span className="badge bg-slate-100 text-slate-600">{row.kind}</span></td><td className="whitespace-nowrap font-medium text-slate-800">{row.from_status ?? "—"} → {row.to_status}</td><td className="text-sm text-slate-600">{row.source}</td><td className="max-w-40 truncate font-mono text-xs text-slate-400" title={row.actor_id ?? "system"}>{row.actor_id ?? "system"}</td><td className="max-w-xs text-sm text-slate-500">{row.note ?? "—"}</td></tr>)}</tbody></table></div></section><p className="text-xs leading-5 text-slate-400">系統稽核只讀取狀態事件，不提供跨品牌修改入口；取消仍保留為狀態異動，不會被刪除。</p></div>;
}
