...
export default async function PlatformAuditPage() {
  ...
  return <div className="space-y-8"><header><p className="eyebrow">跨品牌操作追蹤</p><h1 className="mt-1 text-2xl font-bold text-slate-950">系統稽核</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">集中查看跨品牌的預約、報名與付款狀態異動，協助系統管理者判斷問題是否來自單一品牌、通知流程或付款通知。</p></header><section className="card overflow-hidden"><div className="flex flex-wrap items-end justify-between gap-3 border-b border-slate-100 px-5 py-4 sm:px-6"><div><h2 className="font-semibold text-slate-900">最近 200 筆狀態異動</h2><p className="mt-1 text-sm text-slate-500">事件依時間排序；技術識別資料需要時再展開。</p></div><span className="badge bg-indigo-50 text-indigo-700">跨品牌</span></div><div className="overflow-x-auto"><table className="tbl"><thead><tr><th>時間</th><th>品牌</th><th>類型</th><th>狀態變更</th><th>來源</th><th>操作者</th><th>備註</th></tr></thead><tbody>{rows.length === 0 ? <tr><td colSpan={7} className="py-10 text-center text-sm text-slate-400">尚無跨品牌稽核紀錄。</td></tr> : rows.map((row) => <tr key={`${row.kind}-${row.id}`}><td className="whitespace-nowrap text-xs text-slate-500">{formatDateTime(row.created_at)}</td><td className="font-medium text-slate-800">{brandsById.get(row.clinic_id) ?? "未知品牌"}</td><td><span className="badge bg-slate-100 text-slate-600">{row.kind}</span></td><td className="whitespace-nowrap font-medium text-slate-800">{auditStatusLabel(row.from_status)} → {auditStatusLabel(row.to_status)}</td><td className="text-sm text-slate-600">{auditSourceLabel(row.source)}</td><td className="text-sm text-slate-600">{row.actor_id ? "後台帳號" : "系統自動執行"}<TechnicalDetails className="mt-1" summary="查看識別資料" items={[{ label: "品牌識別碼", value: row.clinic_id }, { label: "來源代碼", value: row.source }, { label: "前一狀態代碼", value: row.from_status ?? "none" }, { label: "目前狀態代碼", value: row.to_status }, { label: "帳號識別碼", value: row.actor_id ?? "system" }, { label: "紀錄識別碼", value: row.reference_id }]} /></td><td className="max-w-xs text-sm text-slate-500">{row.note ?? "—"}</td></tr>)}</tbody></table></div></section><p className="text-xs leading-5 text-slate-400">系統稽核只讀取狀態事件，不提供跨品牌修改入口；取消仍保留為狀態異動，不會被刪除。</p></div>;
}
