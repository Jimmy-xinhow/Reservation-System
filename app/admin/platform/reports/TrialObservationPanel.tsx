...
export async function TrialObservationPanel({ brands, canManage }: { brands: Brand[]; canManage: boolean }) {
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">三品牌試用</p><h2 className="mt-1 text-xl font-bold text-slate-900">三品牌試用觀察</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">同時最多三個品牌；用固定口徑觀察首次可預約、設定退出、權限求助與預約完成率。未發生的行為顯示為「尚無資料」，不使用推估值。</p></div><span className="badge bg-slate-100 text-slate-700">{active.length} / 3 觀察中</span></div>
      {canManage && active.length < 3 && available.length > 0 && <form action={startTrialObservationAction} className="platform-panel grid gap-3 p-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] sm:items-end"><label className="text-sm"><span className="label">加入試用品牌</span><select name="clinic_id" className="input" required defaultValue=""><option value="" disabled>選擇品牌</option>{available.map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label><label className="text-sm"><span className="label">觀察備註</span><input name="notes" className="input" maxLength={1000} placeholder="例如：美容服務新客試用" /></label><SubmitButton className="btn btn-primary min-h-11">開始觀察</SubmitButton></form>}
      {displayed.length === 0 ? <div className="platform-panel px-5 py-10 text-center text-sm text-slate-400">尚未選定試用品牌。請先加入最多三個品牌，觀察資料才會開始累積。</div> : <div className="grid gap-4 xl:grid-cols-3">{displayed.map((observation) => {
