...
export default async function PlatformOperationsPage() {
  ...
  const errors = [activeBrandsError, inactiveBrandsError, failedCrmError, failedAppointmentError, failedRegistrationError, paymentSettingsError, lineSecretError, emailSecretError, paymentSecretError, webhookError].filter(Boolean);
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="card space-y-4 p-5 sm:p-6">
          <div><p className="eyebrow">待處理訊息</p><h2 className="mt-1 text-lg font-bold text-slate-900">待處理通知與訊息</h2><p className="mt-1 text-sm leading-6 text-slate-500">失敗資料會保留在品牌範圍內，由品牌後台或排程重試處理。</p></div>
          ...
          {warningCount > 0 ? <p className="rounded-xl bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800">目前有通知失敗紀錄，請先由品牌後台確認顧客 是否已同意接收行銷訊息、LINE／Email 設定與排程錯誤，再決定是否重試。</p> : <p className="rounded-xl bg-emerald-50 px-4 py-3 text-xs leading-5 text-emerald-800">目前沒有通知失敗紀錄；仍需持續觀察同日新增預約與報名的提醒佇列。</p>}
        </div>
      </section>
