...
export default async function DocumentsPage({ searchParams }: { searchParams: Promise<{ request_id?: string; sign_token?: string }> }) {
  ...
  if (!canViewSensitiveCustomerData(member.role)) return <p className="admin-section p-5 text-sm text-slate-500">目前角色不能查看顧客簽署資料。</p>;
  const params = await searchParams;
  const supabase = await createSupabaseServer();
  const [templates, requestsResult] = await Promise.all([
    fetchAllSupabasePages((from, to) => supabase.from("document_templates").select("id,name,kind,version,active,created_at").eq("clinic_id", member.clinicId).eq("active", true).order("created_at", { ascending: false }).order("id").range(from, to)),
  ...
  return <div className="admin-page">
    ...
    <section className="admin-workbench-grid">
      ...
      <form action={issueDocumentRequestAction} className="admin-section p-4"><h2 className="font-semibold">發出簽署要求</h2><p className="mt-1 text-sm text-slate-500">系統會把目前範本內容固定保存，日後修改範本不會改動已發出的文件。</p><div className="mt-3 space-y-3"><PatientPicker /><label className="block"><span className="label">文件範本</span><select name="template_id" className="input" required defaultValue=""><option value="" disabled>選擇範本</option>{templates.map((template) => <option key={template.id} value={template.id}>{template.name} · 第 {template.version} 版</option>)}</select></label><label className="block"><span className="label">連結有效天數</span><input name="expires_in_days" type="number" min="1" max="30" defaultValue="7" className="input" /></label><SubmitButton className="btn btn-primary" disabled={templates.length === 0}>建立簽署連結</SubmitButton></div></form>
    </section>
