...
export default async function FitnessOperationsPage() {
  ...
  return <div className="admin-page">
    ...
    <section className="admin-metric-strip"><div><span>未來班次</span><strong>{sessionRows.length}</strong></div><div><span>已占名額</span><strong>{attendanceTotal}</strong></div><div><span>總容量</span><strong>{capacityTotal}</strong></div><div><span>有效會籍</span><strong>{subscriptions.filter((item) => ["active", "paused"].includes(item.status)).length}</strong></div></section>
