...
export async function freezeSubscriptionAction(fd: FormData): Promise<void> {
  ...
  const { error } = await adminQuery(createServiceClient().rpc("freeze_patient_subscription", {
  ...
  if (error) throw new Error(error.message.includes("overlaps") ? "這段期間與既有凍結紀錄重疊" : adminErrorMessage(error));
