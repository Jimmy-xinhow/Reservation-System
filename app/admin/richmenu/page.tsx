...
import { LAYOUTS, richMenuTemplate, slotBounds, type Layout, type RichMenuModuleAvailability, type RichMenuTemplateKey, type Slot } from "@/lib/richmenu";
import {
...
function versionDifferences(target: VersionRow, baseline: VersionRow): string[] {
  ...
  if (target.template_key !== baseline.template_key) differences.push(`模板：${baseline.template_key} → ${target.template_key}`);
  if (target.layout !== baseline.layout) differences.push(`版型：${baseline.layout} → ${target.layout}`);
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (JSON.stringify(before ?? null) !== JSON.stringify(after ?? null)) {
      differences.push(`第 ${index + 1} 格：${before ? `${before.label}／${before.action}` : "無"} → ${after ? `${after.label}／${after.action}` : "無"}`);
    }
  }
  return differences;
}
...
export default async function RichMenuPage({
  ...
  try {
    ...
    lineReadiness = [
      ...
      { label: "destination 與 access token 對應", ready: Boolean(context.destination && accessToken) },
      { label: "LINE Login Channel", ready: Boolean(context.loginChannelId) },
      { label: "LIFF ID", ready: Boolean(context.liffId) },
      { label: "正式渠道驗證", ready: context.verificationStatus === "ready" },
    ];
  ...
  if (insightVersion && accessToken) {
    try {
      ...
      insightError = caught instanceof Error ? caught.message : "讀取 Rich Menu 洞察失敗";
    }
  }
  ...
  return (
    <div className="space-y-6">
      <header><p className="eyebrow">Customer entry</p><h1 className="mt-1 text-xl font-bold text-slate-900">Rich Menu 版本與發布</h1><p className="mt-1 text-sm text-slate-500">先建立版本，再設定頁籤、顯示期間與成效檢視；任何草稿都不會直接改動線上選單。</p></header>

      {oneParam("err") && <p className="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700">操作失敗：{oneParam("err")}</p>}
      {oneParam("ok") && <p className="rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-700">已成功發布 Rich Menu。</p>}
      ...
      {oneParam("alias_saved") && <p className="rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-700">Alias 已同步至 LINE。</p>}
      {oneParam("alias_removed") && <p className="rounded-xl bg-blue-50 px-4 py-3 text-sm text-blue-700">Alias 已從 LINE 移除。</p>}
      ...
      <section className="card p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><h2 className="font-semibold text-slate-900">發布就緒檢查</h2><p className="mt-1 text-xs text-slate-500">未全部通過時，發布、Alias 與排程操作都應視為未就緒。</p></div><div className="flex flex-wrap gap-2">{lineReadiness.map((item) => <span key={item.label} className={`badge ${item.ready ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{item.ready ? "✓" : "!"} {item.label}</span>)}</div></div></section>

      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(20rem,.8fr)]">
        <div className="space-y-6">
          <RichMenuEditor initialLayout={layout} initialChatBar={chatBar} initialSlots={slots} initialName={draft?.name ?? "新的 Rich Menu 草稿"} initialTemplate={draft?.template_key ?? "mixed"} availability={availability} messages={messages} aliases={activeAliases.map(({ alias_id, label }) => ({ alias_id, label }))} saveAction={saveRichMenuAction} />
          <PublishForm
            width={spec.width}
            height={spec.height}
            layout={layout}
        ...
        <div className="space-y-6">
          <section className="card p-5"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-slate-900">目前線上版本</h2><span className={`badge ${publishedId ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{publishedId ? "已發布" : "未發布"}</span></div>{publishedId ? <div className="mt-4 space-y-3">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={`/api/admin/richmenu-image${publishedVersionId ? `?version=${encodeURIComponent(publishedVersionId)}` : ""}`} alt="目前已發布的 Rich Menu" className="w-full rounded-xl border border-slate-200" /><form action={unpublishRichMenuAction}><SubmitButton className="btn btn-danger w-full">取消發布</SubmitButton></form></div> : <p className="mt-3 text-sm text-slate-500">尚未設定 LINE 預設 Rich Menu。</p>}</section>
          <section className="card p-5"><h2 className="font-semibold text-slate-900">版本紀錄</h2><div className="mt-4 space-y-3">{versions.length === 0 ? <p className="text-sm text-slate-400">尚無版本。請先另存第一份草稿。</p> : versions.map((version) => <article key={version.id} className={`rounded-xl border p-4 ${version.id === draft?.id ? "border-brand-300 bg-brand-50/50" : "border-slate-200"}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium text-slate-900">v{version.version_no} · {version.name}</p><p className="mt-1 text-xs text-slate-500">{version.template_key} · {LAYOUTS[version.layout]?.label ?? version.layout}{version.source_version_id ? " · 複製版本" : ""}</p></div><span className="badge bg-slate-100 text-slate-600">{STATUS_LABEL[version.status]}</span></div>{version.validation_errors?.length > 0 && <p className="mt-2 text-xs text-red-600">{version.validation_errors.join("；")}</p>}<div className="mt-3 flex flex-wrap gap-2"><a href={`/admin/richmenu?draft=${encodeURIComponent(version.id)}`} className="btn btn-secondary px-3 py-1.5 text-xs">檢視／另存</a>{draft && version.id !== draft.id && <a href={`/admin/richmenu?draft=${encodeURIComponent(draft.id)}&compare=${encodeURIComponent(version.id)}`} className="btn btn-secondary px-3 py-1.5 text-xs">與草稿比較</a>}<form action={cloneRichMenuVersionAction}><input type="hidden" name="version_id" value={version.id} /><SubmitButton className="btn btn-secondary px-3 py-1.5 text-xs">複製為新草稿</SubmitButton></form>{version.line_rich_menu_id && version.id !== publishedVersionId && <form action={rollbackRichMenuVersionAction}><input type="hidden" name="version_id" value={version.id} /><SubmitButton className="btn btn-secondary px-3 py-1.5 text-xs">回復此版本</SubmitButton></form>}</div></article>)}</div></section>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="card p-5"><div><h2 className="font-semibold text-slate-900">Alias 頁籤</h2><p className="mt-1 text-sm text-slate-500">Alias 只能指向同一品牌渠道內、已上傳圖片的版本；建立後可在版本區塊選擇「切換 Rich Menu 頁籤」。</p></div><form action={syncRichMenuAliasAction} className="mt-4 grid gap-3 sm:grid-cols-3"><label className="text-sm"><span className="label">Alias ID</span><input name="alias_id" required pattern="[a-z0-9_-]{1,32}" maxLength={32} placeholder="main_menu" className="input" /></label><label className="text-sm"><span className="label">管理名稱</span><input name="label" required maxLength={40} placeholder="主選單" className="input" /></label><label className="text-sm"><span className="label">對應版本</span><select name="version_id" required className="input"><option value="">請選擇</option>{lineBackedVersions.map((version) => <option key={version.id} value={version.id}>v{version.version_no} · {version.name}</option>)}</select></label><div className="sm:col-span-3"><SubmitButton disabled={!lineReady || lineBackedVersions.length === 0} className="btn btn-primary">同步 Alias</SubmitButton></div></form><div className="mt-5 space-y-2">{aliases.length === 0 ? <p className="text-sm text-slate-400">尚未建立 Alias。</p> : aliases.map((alias) => <div key={alias.id} className="flex flex-col gap-2 rounded-xl border border-slate-200 p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-medium text-slate-800">{alias.label} <span className="font-mono text-xs text-slate-400">{alias.alias_id}</span></p><p className="mt-1 text-xs text-slate-500">{alias.status === "ready" ? `對應 v${versions.find((version) => version.id === alias.version_id)?.version_no ?? "?"}` : alias.status}{alias.last_error ? ` · ${alias.last_error}` : ""}</p></div>{alias.status !== "removed" && <form action={removeRichMenuAliasAction}><input type="hidden" name="alias_id" value={alias.alias_id} /><SubmitButton className="btn btn-secondary px-3 py-1.5 text-xs">移除</SubmitButton></form>}</div>)}</div></section>

        <section className="card p-5"><div><h2 className="font-semibold text-slate-900">顯示期間與排程</h2><p className="mt-1 text-sm text-slate-500">時間以台北時間輸入；開始時切換到指定版本，結束後回復排程開始前的版本。</p></div><form action={createRichMenuScheduleAction} className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-sm sm:col-span-2"><span className="label">顯示版本</span><select name="version_id" required className="input"><option value="">請選擇</option>{lineBackedVersions.map((version) => <option key={version.id} value={version.id}>v{version.version_no} · {version.name}</option>)}</select></label><label className="text-sm"><span className="label">開始（Asia/Taipei）</span><input name="starts_at" type="datetime-local" required className="input" /></label><label className="text-sm"><span className="label">結束（Asia/Taipei）</span><input name="ends_at" type="datetime-local" required className="input" /></label><div className="sm:col-span-2"><SubmitButton disabled={!lineReady || lineBackedVersions.length === 0} className="btn btn-primary">建立排程</SubmitButton></div></form><div className="mt-5 space-y-2">{schedules.length === 0 ? <p className="text-sm text-slate-400">尚無顯示排程。</p> : schedules.map((schedule) => <div key={schedule.id} className="rounded-xl border border-slate-200 p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-medium text-slate-800">v{versions.find((version) => version.id === schedule.version_id)?.version_no ?? "?"} · {SCHEDULE_STATUS[schedule.status]}</p><p className="mt-1 text-xs text-slate-500">{formatTaipei(schedule.starts_at)} ～ {formatTaipei(schedule.ends_at)} · 嘗試 {schedule.attempt_count} 次</p>{schedule.last_error && <p className="mt-1 text-xs text-red-600">{schedule.last_error}</p>}</div>{schedule.status === "scheduled" && <form action={cancelRichMenuScheduleAction}><input type="hidden" name="schedule_id" value={schedule.id} /><SubmitButton className="btn btn-secondary px-3 py-1.5 text-xs">取消排程</SubmitButton></form>}</div></div>)}</div></section>
      </div>

      <section className="card p-5"><div><h2 className="font-semibold text-slate-900">曝光、點擊與轉換</h2><p className="mt-1 text-sm text-slate-500">LINE 提供整份選單曝光與各點擊區域統計；每個區域會同步看見整份選單曝光。預約／報名轉換來自平台匿名漏斗事件，不含姓名、電話或 LINE user ID。</p></div><form method="get" className="mt-4 grid gap-3 sm:grid-cols-4"><label className="text-sm sm:col-span-2"><span className="label">版本</span><select name="insight_version" required defaultValue={insightVersion?.id ?? ""} className="input"><option value="">請選擇</option>{lineBackedVersions.map((version) => <option key={version.id} value={version.id}>v{version.version_no} · {version.name}</option>)}</select></label><label className="text-sm"><span className="label">開始日期</span><input name="insight_from" type="date" defaultValue={insightFrom} required className="input" /></label><label className="text-sm"><span className="label">結束日期</span><input name="insight_to" type="date" defaultValue={insightTo} required className="input" /></label><div className="sm:col-span-4"><button type="submit" disabled={!lineReady || lineBackedVersions.length === 0} className="btn btn-primary">讀取官方洞察</button></div></form>{insightError && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{insightError}</p>}{insightVersion && insight && !insight.impression && !insight.clicks && <p className="mt-4 rounded-xl bg-amber-50 p-3 text-sm text-amber-800">LINE 因隱私門檻未回傳統計明細；指定期間的點擊不重複使用者少於 20 人時，只會回傳 Rich Menu ID。</p>}{insightVersion && insight && (insight.impression || insight.clicks) && <div className="mt-5 overflow-x-auto"><table className="tbl"><thead><tr><th>區域</th><th>選單曝光</th><th>區域點擊</th><th>預約完成</th><th>報名完成</th></tr></thead><tbody>{insightVersion.slots.map((slot, index) => { const bounds = insightBounds[index]; const click = insight.clicks?.find((item) => item.bounds.x === bounds.x && item.bounds.y === bounds.y && item.bounds.width === bounds.width && item.bounds.height === bounds.height); const conversion = conversions.get(index + 1) ?? { booking: 0, registration: 0 }; return <tr key={`${insightVersion.id}-${index}`}><td><p className="font-medium text-slate-800">第 {index + 1} 格 · {slot.label}</p><p className="text-xs text-slate-400">{slot.action}</p></td><td>{insight.impression?.metrics.count ?? "—"}<div className="text-xs text-slate-400">{insight.impression ? `${insight.impression.metrics.uniqueUsers} 人` : ""}</div></td><td>{click?.metrics.count ?? 0}<div className="text-xs text-slate-400">{click ? `${click.metrics.uniqueUsers} 人` : ""}</div></td><td>{conversion.booking}</td><td>{conversion.registration}</td></tr>; })}</tbody></table></div>}</section>
    </div>
  );
}
