...
const REGISTRATION_STATUS_LABEL: Record<string, string> = { pending: "待確認", confirmed: "已確認", attended: "已出席", no_show: "未到", waitlisted: "候補", cancelled: "已取消" };
const REGISTRATION_STATUS_COLOR: Record<string, string> = { pending: "bg-brand-500", confirmed: "bg-accent-500", attended: "bg-slate-400", no_show: "bg-amber-500", waitlisted: "bg-violet-400", cancelled: "bg-red-400" };
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const dashboardFocus = (["booking", "registration", "mixed"].includes(String(productSettings.dashboard_focus)) ? productSettings.dashboard_focus : "mixed") as "booking" | "registration" | "mixed";
  ...
  const [
    ...
    role === "provider" ? Promise.resolve({ data: [] as PurchaseOrderRow[], error: null }) : fetchAllSupabasePages((from, to) => supabase.from("purchase_orders").select("status, purchase_order_items:purchase_order_items!purchase_order_items_purchase_order_id_fkey(quantity, unit_cost)").eq("clinic_id", clinicId).in("status", ["ordered", "received"]).gte("created_at", monthStartIso).lte("created_at", todayEndIso).order("created_at").order("id").range(from, to)).then((data) => ({ data, error: null })),
  ...
  const registrationStatusCounts = countBy(registrations, (item) => item.status);
...
function StatusBars({ labels, colors, counts, total }: { labels: Record<string, string>; colors: Record<string, string>; counts: Record<string, number>; total: number }) { const denominator = total || 1; return <div className="space-y-2.5">{Object.keys(labels).map((key) => { const count = counts[key] ?? 0; return <div key={key} className="flex items-center gap-3 text-sm"><span className="w-14 shrink-0 text-slate-600">{labels[key]}</span><div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100"><div className={`h-full ${colors[key]}`} style={{ width: `${count / denominator * 100}%` }} /></div><span className="w-8 shrink-0 text-right text-slate-500">{count}</span></div>; })}</div>; }
