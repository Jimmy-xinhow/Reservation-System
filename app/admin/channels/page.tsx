...
const LABEL: Record<string, string> = { line: "LINE 訊息功能", liff: "LINE 顧客入口", email: "Email 通知", payment: "綠界／藍新付款", domain: "短網址與自訂網域" };
...
export default async function ChannelsPage({ searchParams }: { searchParams: Promise<{ tested?: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">上線前檢查</p><h1 className="mt-1 text-2xl font-bold text-slate-900">通知與付款測試</h1><p className="mt-2 max-w-3xl text-base leading-7 text-slate-600">一次檢查 LINE、Email、標準付款與公開網址是否已完成基本設定；畫面只顯示檢查結果，不會讀出或保存密鑰。</p></div><form action={runChannelTestsAction}><SubmitButton className="btn btn-primary min-h-11">執行全部檢查</SubmitButton></form></header>
      ...
      <p className="text-sm leading-6 text-slate-600">這裡只檢查系統設定與可安全查詢的連線狀態。正式上線前，仍要由測試人員用 LINE 手機實際登入、點擊圖文選單、接收訊息並完成一筆測試付款。</p>
    </div>
  );
}
