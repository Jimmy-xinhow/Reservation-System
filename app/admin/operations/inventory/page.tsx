...
interface MovementRow { id: string; kind: string; quantity: number; stock_after: number; note: string | null; created_at: string; inventory_items: Relation<{ clinic_id: string; name: string; unit: string }>; }
...
export default async function InventoryOperationsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const [params, inventory, movementsResult] = await adminQuery(Promise.all([
    searchParams,
    fetchAllSupabasePages((from, to) => service.from("inventory_items").select("id, sku, name, unit, stock_on_hand, reorder_level, retail_price, active").eq("clinic_id", clinicId).eq("active", true).order("name").order("id").range(from, to)) as Promise<InventoryRow[]>,
    service.from("inventory_movements").select("id, kind, quantity, stock_after, note, created_at, inventory_items!inventory_movements_clinic_item_fkey(clinic_id, name, unit)").eq("clinic_id", clinicId).order("created_at", { ascending: false }).order("id", { ascending: false }).limit(30),
  ]));
  if (movementsResult.error) throw new Error(adminErrorMessage(movementsResult.error.message));
  ...
  const visibleInventory = inventory.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
