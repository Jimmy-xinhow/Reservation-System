...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ date?: string; doctor?: string; selected?: string }> }) {
  ...
  for (const appointment of appointments) {
    const key = appointment.doctor_id ?? "unassigned";
    byDoctor.set(key, [...(byDoctor.get(key) ?? []), appointment]);
  }
  const unassignedAppointments = byDoctor.get("unassigned") ?? [];
  const calendarColumns = doctors.length + (unassignedAppointments.length > 0 ? 1 : 0);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div><div className="eyebrow">預約營運</div><h1 className="text-2xl font-bold tracking-tight text-slate-900">預約日曆</h1><p className="mt-1 text-sm text-slate-500">以時間軸查看服務人員工作量與預約狀態。</p></div>
      ...
      {calendarColumns === 0 ? <section className="card p-8 text-center"><p className="font-medium text-slate-700">尚未設定可用的服務人員</p><p className="mt-1 text-sm text-slate-500">先建立人員與服務時段，日曆才會顯示可操作欄位。</p><Link href="/admin/schedules" className="btn btn-primary mt-4">前往服務排程</Link></section> : <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="card overflow-hidden"><div className="border-b border-slate-200 bg-white px-4 py-3 sm:px-5"><div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold text-slate-900">人員工作時間軸</h2><p className="mt-0.5 text-xs text-slate-400">{appointments.length} 筆預約 · 點擊卡片查看詳情</p></div><Link href={`/admin?date=${date}`} className="text-sm font-medium text-brand-600 hover:underline">切換列表</Link></div></div><div className="overflow-x-auto"><div className="min-w-[760px]"><div className="grid border-b border-slate-200 bg-slate-50" style={{ gridTemplateColumns: `72px repeat(${calendarColumns}, minmax(180px, 1fr))` }}><div className="border-r border-slate-200 px-3 py-3 text-xs font-semibold text-slate-400">時間</div>{doctors.map((doctor) => <div key={doctor.id} className="border-r border-slate-200 px-3 py-3 text-sm font-semibold text-slate-700">{doctor.name}</div>)}{unassignedAppointments.length > 0 && <div className="border-r border-slate-200 bg-amber-50/40 px-3 py-3 text-sm font-semibold text-amber-800">未指定服務人員</div>}</div><div className="grid" style={{ gridTemplateColumns: `72px repeat(${calendarColumns}, minmax(180px, 1fr))` }}><div className="relative border-r border-slate-200 bg-slate-50" style={{ height: gridHeight }}>{timeRows.map((minute) => <div key={minute} className="absolute right-2 -translate-y-1/2 text-[11px] tabular-nums text-slate-400" style={{ top: (minute - DAY_START) * PX_PER_MINUTE }}>{String(Math.floor(minute / 60)).padStart(2, "0")}:{String(minute % 60).padStart(2, "0")}</div>)}</div>{doctors.map((doctor) => <CalendarColumn key={doctor.id} doctor={doctor} appointments={byDoctor.get(doctor.id) ?? []} selectedDate={date} selectedDoctor={selectedDoctor} selectedId={selected?.id} gridHeight={gridHeight} timeRows={timeRows} />)}{unassignedAppointments.length > 0 && <CalendarColumn doctor={{ id: "unassigned", name: "未指定服務人員" }} appointments={unassignedAppointments} selectedDate={date} selectedDoctor={selectedDoctor} selectedId={selected?.id} gridHeight={gridHeight} timeRows={timeRows} />}</div></div></div></section>
        <aside className="card h-fit overflow-hidden"><div className="border-b border-slate-200 px-5 py-4"><div className="eyebrow mb-1">目前選取</div><h2 className="font-semibold text-slate-900">預約詳情</h2></div>{!selected ? <div className="px-5 py-10 text-center text-sm leading-6 text-slate-400">選取左側預約卡片，查看顧客、服務與狀態。</div> : <div className="space-y-5 p-5"><div className="flex items-start justify-between gap-3"><div><div className="text-lg font-semibold text-slate-900">{selected.patients?.name ?? "未命名顧客"}</div><div className="mt-1 text-sm text-slate-500">{selected.services?.name ?? "未指定服務"}</div></div><span className={`badge ${STATUS_COLOR[selected.status] ?? "bg-slate-100 text-slate-600"}`}>{STATUS_LABEL[selected.status] ?? "其他狀態"}</span></div><dl className="space-y-3 text-sm"><Detail label="日期" value={date} /><Detail label="時間" value={`${formatTime(selected.start_at)} – ${formatTime(selected.end_at)}`} /><Detail label="人員" value={selected.doctors?.name ?? "未指定"} /><Detail label="類型" value={selected.visit_type === "first" ? "首次服務" : "再次服務"} /><Detail label="訂金" value={auditStatusLabel(selected.deposit_status)} /><Detail label="電話" value={showPii ? (selected.patients?.phone ?? "未提供") : maskPhone(selected.patients?.phone)} /></dl><div className="grid gap-2"><Link href={`/admin?date=${date}`} className="btn btn-secondary w-full">編輯／重新排程</Link>{canOperateHere && (selected.status === "booked" || selected.status === "confirmed") && <>{selected.status === "booked" && <form action={setStatusAction}><input type="hidden" name="id" value={selected.id} /><input type="hidden" name="status" value="confirmed" /><SubmitButton className="btn btn-primary w-full">確認預約</SubmitButton></form>}<form action={setStatusAction}><input type="hidden" name="id" value={selected.id} /><input type="hidden" name="status" value="done" /><SubmitButton className="btn btn-secondary w-full">標記完成</SubmitButton></form><form action={setStatusAction}><input type="hidden" name="id" value={selected.id} /><input type="hidden" name="status" value="no_show" /><SubmitButton className="btn btn-secondary w-full">標記未到</SubmitButton></form><form action={cancelAppointmentAction}><input type="hidden" name="id" value={selected.id} /><SubmitButton className="btn btn-danger w-full">取消預約</SubmitButton></form></>}</div></div>}</aside>
      </div>}
    </div>
  );
}
