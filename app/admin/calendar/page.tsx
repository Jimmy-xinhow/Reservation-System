  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div><div className="eyebrow">Operations / Calendar</div><h1 className="text-2xl font-bold tracking-tight text-slate-900">預約日曆</h1><p className="mt-1 text-sm text-slate-500">以營運後台的時間軸查看服務人員工作量與預約狀態。</p></div>
      ...
      {doctors.length === 0 ? <section className="card p-8 text-center"><p className="font-medium text-slate-700">尚未設定可用的服務人員</p><p className="mt-1 text-sm text-slate-500">先建立人員與服務時段，日曆才會顯示可操作欄位。</p><Link href="/admin/schedules" className="btn btn-primary mt-4">前往服務排程</Link></section> : <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <aside className="card h-fit overflow-hidden"><div className="border-b border-slate-200 px-5 py-4"><div className="eyebrow mb-1">Appointment detail</div><h2 className="font-semibold text-slate-900">預約詳情</h2></div>{!selected ? <div className="px-5 py-10 text-center text-sm leading-6 text-slate-400">選取左側預約卡片，查看顧客、服務與狀態。</div> : <div className="space-y-5 p-5"><div className="flex items-start justify-between gap-3"><div><div className="text-lg font-semibold text-slate-900">{selected.patients?.name ?? "未命名顧客"}</div><div className="mt-1 text-sm text-slate-500">{selected.services?.name ?? "未指定服務"}</div></div><span className={`badge ${STATUS_COLOR[selected.status] ?? "bg-slate-100 text-slate-600"}`}>{STATUS_LABEL[selected.status] ?? selected.status}</span></div><dl className="space-y-3 text-sm"><Detail label="日期" value={date} /><Detail label="時間" value={`${formatTime(selected.start_at)} – ${formatTime(selected.end_at)}`} /><Detail label="人員" value={selected.doctors?.name ?? "未指定"} /><Detail label="類型" value={selected.visit_type === "first" ? "首次服務" : "再次服務"} /><Detail label="訂金" value={selected.deposit_status === "none" ? "未設定" : selected.deposit_status} /><Detail label="電話" value={showPii ? (selected.patients?.phone ?? "未提供") : maskPhone(selected.patients?.phone)} /></dl><div className="grid gap-2"><Link href={`/admin?date=${date}`} className="btn btn-secondary w-full">編輯／重新排程</Link>{canOperateHere && (selected.status === "booked" || selected.status === "confirmed") && <>{selected.status === "booked" && <form action={setStatusAction}><input type="hidden" name="id" value={selected.id} /><input type="hidden" name="status" value="confirmed" /><SubmitButton className="btn btn-primary w-full">確認預約</SubmitButton></form>}<form action={setStatusAction}><input type="hidden" name="id" value={selected.id} /><input type="hidden" name="status" value="done" /><SubmitButton className="btn btn-secondary w-full">標記完成</SubmitButton></form><form action={setStatusAction}><input type="hidden" name="id" value={selected.id} /><input type="hidden" name="status" value="no_show" /><SubmitButton className="btn btn-secondary w-full">標記未到</SubmitButton></form><form action={cancelAppointmentAction}><input type="hidden" name="id" value={selected.id} /><SubmitButton className="btn btn-danger w-full">取消預約</SubmitButton></form></>}</div></div>}</aside>
      </div>}
    </div>
  );
}
