...
async function record(clinicId: string, userId: string, type: AttendanceEventType, method: "button" | "qr") {
  const { error } = await adminQuery(createServiceClient().from("attendance_events").insert({ clinic_id: clinicId, user_id: userId, event_type: type, method }));
  if (error) throw new Error(adminErrorMessage(`打卡失敗：${error.message}`));
...
export async function recordButtonAttendanceAction(fd: FormData): Promise<void> {
  ...
  const { data, error } = await adminQuery(service.from("attendance_settings").select("click_enabled").eq("clinic_id", member.clinicId).maybeSingle());
...
export async function recordQrAttendanceAction(fd: FormData): Promise<void> {
  ...
  const service = createServiceClient();
  const [{ data: settings, error: settingsError }, { data: validToken, error: tokenError }] = await adminQuery(Promise.all([
...
export async function saveAttendanceSettingsAction(fd: FormData): Promise<void> {
  ...
  const { error } = await adminQuery(createServiceClient().from("attendance_settings").upsert({
  ...
  if (error) throw new Error(adminErrorMessage(`儲存打卡設定失敗：${error.message}`));
  revalidatePath("/admin/attendance");
  revalidatePath("/admin/dashboard");
}
...
export async function saveAttendanceStaffAction(fd: FormData): Promise<void> {
  ...
  const service = createServiceClient();
  const { data: clinicMember, error: memberError } = await adminQuery(service.from("clinic_members").select("user_id").eq("clinic_id", member.clinicId).eq("user_id", userId).maybeSingle());
  if (memberError || !clinicMember) throw new Error("員工不屬於目前品牌");
  const { error } = await adminQuery(service.from("attendance_staff").upsert({ clinic_id: member.clinicId, user_id: userId, display_name: displayName || null, line_user_id: lineUserId || null, active: fd.get("active") === "on" }, { onConflict: "clinic_id,user_id" }));
  if (error?.code === "23505") throw new Error("這個 LINE 帳號已綁定其他員工");
  if (error) throw new Error(adminErrorMessage(`儲存員工綁定失敗：${error.message}`));
  revalidatePath("/admin/attendance");
  revalidatePath("/admin/dashboard");
}
