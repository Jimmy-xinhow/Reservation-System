    ...
    <div className="admin-page">
      ...
      <div className="admin-table-shell admin-table-mobile-cards">
        <table className="tbl">
          ...
          <tbody>
        {rows.length === 0 ? <tr><td colSpan={7} className="py-10 text-center text-sm text-slate-400" data-mobile-empty="true">目前沒有符合條件的報名。</td></tr> : rows.map((row) => {
          const event = Array.isArray(row.events) ? row.events[0] : row.events;
          const session = Array.isArray(row.event_sessions) ? row.event_sessions[0] : row.event_sessions;
          return <tr key={row.id}><td data-label="報名編號"><code className="text-xs font-semibold text-slate-700">{row.registration_no}</code></td><td data-label="活動／場次"><div className="font-medium text-slate-900">{event?.title ?? "未命名活動"}</div><div className="mt-0.5 text-xs text-slate-500">{session ? `${session.name} · ${formatEventDate(session.start_at)}` : "未設定場次"}</div></td><td data-label="顧客">{showPii ? <><div className="font-medium text-slate-800">{row.name}</div><div className="text-xs text-slate-500">{row.phone}{row.email ? ` · ${row.email}` : ""}</div></> : <span className="text-xs text-slate-400">聯絡資料已遮蔽</span>}</td><td data-label="金額">{formatAmount(Number(row.amount))}{row.discount_amount > 0 && <div className="text-xs text-emerald-700">已折 {formatAmount(row.discount_amount)}</div>}</td><td data-label="狀態"><div className="flex flex-col items-start gap-1"><span className="badge bg-brand-50 text-brand-700">{registrationStatusLabel(row.status)}</span><span className="text-xs text-slate-500">{paymentStatusLabel(row.payment_status)}</span></div></td><td className="text-xs text-slate-500" data-label="建立時間">{formatEventDate(row.created_at)}</td><td data-label="操作">{canOperate(member.role) && <div className="flex flex-wrap gap-1">{["pending", "confirmed", "attended"].includes(row.status) && <Link href={`/admin/checkout?registration_id=${row.id}`} className="admin-inline-action text-brand-700">結帳</Link>}{["pending", "confirmed", "waitlisted"].includes(row.status) && <form action={cancelRegistrationAdminAction}><input type="hidden" name="id" value={row.id} /><SubmitButton className="admin-inline-action text-red-700">取消</SubmitButton></form>}{row.status === "confirmed" && <form action={markRegistrationNoShowAction}><input type="hidden" name="id" value={row.id} /><SubmitButton className="admin-inline-action">未到</SubmitButton></form>}</div>}</td></tr>;
        })}
          </tbody>
        </table>
      </div>
