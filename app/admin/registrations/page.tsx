...
import { cancelRegistrationAdminAction } from "./actions";
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        {rows.length === 0 ? <div className="card p-8 text-center text-sm text-slate-400">目前沒有符合條件的報名。</div> : rows.map((row) => {
          ...
          return <article key={row.id} className="card space-y-3 p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="font-semibold text-slate-900">{event?.title ?? "未命名活動"}</div><div className="mt-1 text-sm text-slate-500">{session ? `${session.name} · ${formatEventDate(session.start_at)}` : "未設定場次"}</div></div><div className="flex gap-2 text-xs"><span className="badge bg-brand-50 text-brand-700">{row.status}</span><span className="badge bg-slate-100 text-slate-600">{row.payment_status}</span></div></div><div className="grid gap-2 text-sm text-slate-600 sm:grid-cols-3"><div>報名編號：<code>{row.registration_no}</code></div><div>金額：{formatAmount(Number(row.amount))}</div><div>建立：{formatEventDate(row.created_at)}</div></div>{showPii ? <div className="grid gap-2 text-sm text-slate-600 sm:grid-cols-3"><div>姓名：{row.name}</div><div>電話：{row.phone}</div><div>Email：{row.email ?? "—"}</div></div> : <p className="text-xs text-slate-400">目前角色僅顯示必要報名資訊，已遮蔽聯絡資料。</p>}{canOperate(member.role) && ["pending", "confirmed", "waitlisted"].includes(row.status) && <form action={cancelRegistrationAdminAction}><input type="hidden" name="id" value={row.id} /><SubmitButton className="btn btn-secondary text-xs">取消報名</SubmitButton></form>}</article>;
        })}
      </div>
    </div>
  );
