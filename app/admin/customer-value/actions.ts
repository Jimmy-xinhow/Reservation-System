...
export async function adjustWalletAction(fd: FormData): Promise<void> {
  ...
  const { error } = await adminQuery(createServiceClient().rpc("adjust_customer_wallet", { p_clinic_id: member.clinicId, p_actor_user_id: member.user.id, p_patient_id: text(fd, "patient_id"), p_amount_delta: delta, p_kind: kind, p_note: text(fd, "note") || null, p_sales_order_id: null, p_idempotency_key: null }));
  if (error) throw new Error(error.message.includes("insufficient") ? "顧客儲值餘額不足" : adminErrorMessage(error));
  refresh();
}
...
export async function adjustPointsAction(fd: FormData): Promise<void> {
  ...
  const { error } = await adminQuery(createServiceClient().rpc("adjust_loyalty_points", { p_clinic_id: member.clinicId, p_actor_user_id: member.user.id, p_patient_id: text(fd, "patient_id"), p_points_delta: delta, p_kind: kind, p_note: text(fd, "note") || null, p_sales_order_id: null, p_idempotency_key: null }));
  if (error) throw new Error(error.message.includes("insufficient") ? "顧客點數不足" : adminErrorMessage(error));
  refresh();
}
...
export async function toggleSubscriptionPlanAction(fd: FormData): Promise<void> {
  const member = await requireAdmin();
  const { error } = await adminQuery(createServiceClient().from("subscription_plans").update({ active: text(fd, "active") !== "true" }).eq("id", text(fd, "id")).eq("clinic_id", member.clinicId));
  if (error) throw new Error(adminErrorMessage(error));
  refresh();
}
...
export async function createPatientSubscriptionAction(fd: FormData): Promise<void> {
  const member = await requireOperator();
  const { error } = await adminQuery(createServiceClient().rpc("create_patient_subscription", { p_clinic_id: member.clinicId, p_actor_user_id: member.user.id, p_patient_id: text(fd, "patient_id"), p_plan_id: text(fd, "plan_id"), p_note: text(fd, "note") || null }));
  if (error) throw new Error(error.message.includes("duplicate key") ? "顧客已有這個有效訂閱方案" : adminErrorMessage(error));
  refresh();
}
...
export async function setPatientSubscriptionStatusAction(fd: FormData): Promise<void> {
  ...
  const { error } = await adminQuery(createServiceClient().rpc("set_patient_subscription_status", { p_clinic_id: member.clinicId, p_actor_user_id: member.user.id, p_subscription_id: text(fd, "id"), p_status: status }));
  if (error) throw new Error(adminErrorMessage(error));
  refresh();
}
