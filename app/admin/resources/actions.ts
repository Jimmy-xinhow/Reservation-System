...
export async function createResourceAction(formData: FormData): Promise<void> {
  ...
  const { error } = await adminQuery(createServiceClient().from("service_resources").insert({ clinic_id: clinicId, name, kind, capacity: positiveInt(formData, "capacity", 1) }));
...
export async function toggleResourceAction(formData: FormData): Promise<void> {
  ...
  const { error } = await adminQuery(createServiceClient().from("service_resources").update({ active: !active }).eq("id", id).eq("clinic_id", clinicId));
  if (error) throw new Error(adminErrorMessage(error));
  revalidatePath("/admin/resources");
}
...
export async function assignResourceAction(formData: FormData): Promise<void> {
  ...
  const [{ data: service }, { data: resource }] = await adminQuery(Promise.all([
  ...
  const { error } = await adminQuery(svc.from("service_resource_assignments").upsert({ clinic_id: clinicId, service_id: serviceId, resource_id: resourceId, quantity: positiveInt(formData, "quantity", 1) }, { onConflict: "service_id,resource_id" }));
  if (error) throw new Error(adminErrorMessage(error));
  revalidatePath("/admin/resources");
}
...
export async function removeAssignmentAction(formData: FormData): Promise<void> {
  const { clinicId } = await requireAdmin();
  const id = value(formData, "id");
  const { error } = await adminQuery(createServiceClient().from("service_resource_assignments").delete().eq("id", id).eq("clinic_id", clinicId));
  if (error) throw new Error(adminErrorMessage(error));
  revalidatePath("/admin/resources");
}
