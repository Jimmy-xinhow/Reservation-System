...
export default async function NewSalesOrderPage({
  ...
  searchParams: Promise<{ appointment_id?: string; registration_id?: string }>;
}) {
  ...
  if (params.appointment_id && !appointments.some((row) => row.id === params.appointment_id)) {
    const { data, error } = await supabase.from("appointments").select("id, start_at, patients(name), services(name, price)").eq("clinic_id", member.clinicId).eq("id", params.appointment_id).in("status", ["booked", "confirmed", "done"]).maybeSingle();
    if (error) throw new Error(error.message);
    if (data) appointments.unshift(data as unknown as AppointmentOption);
  }
  if (params.registration_id && !registrations.some((row) => row.id === params.registration_id)) {
    const { data, error } = await supabase.from("registrations").select("id, registration_no, name, amount, events(title)").eq("clinic_id", member.clinicId).eq("id", params.registration_id).in("status", ["pending", "confirmed", "attended"]).maybeSingle();
    if (error) throw new Error(error.message);
  ...
  const options: CheckoutSourceOption[] = [
    ...appointments.map((appointment) => {
      ...
      return { value: `appointment:${appointment.id}`, kind: "appointment" as const, label: `${dateTime(appointment.start_at)} · ${patient?.name ?? "顧客"} · ${service?.name ?? "一般服務"}`, amount: Number(service?.price ?? 0) };
    }),
  ...
  const requestedSource = params.appointment_id ? `appointment:${params.appointment_id}` : params.registration_id ? `registration:${params.registration_id}` : "";
  const defaultSource = options.some((option) => option.value === requestedSource) ? requestedSource : "";

  return (
    <div className="admin-page checkout-create-page">
      <div className="admin-page-header">
        <div><p className="eyebrow">結帳中心</p><h1 className="admin-page-title">建立銷售單</h1><p className="admin-page-description">單獨完成來源、成交金額與折扣設定，再進入品項與收款。</p></div>
        <Link href="/admin/checkout" className="btn btn-secondary">← 返回結帳中心</Link>
      </div>
      <CreateSalesOrderForm options={options} defaultSource={defaultSource} />
    </div>
  );
}
