...
const PAGE_SIZE = 50;
const ORDER_PROJECTION = "id, order_no, appointment_id, registration_id, patient_id, status, subtotal, discount_amount, total_amount, paid_amount, created_at, patients(name), sales_order_items(id, kind, name, quantity, unit_price, line_total, created_at), sales_payments(id, method, amount, reference, received_at)";
...
export default async function CheckoutPage({ searchParams }: { searchParams: Promise<{ order_id?: string; page?: string; modal?: string; appointment_id?: string; registration_id?: string }> }) {
  ...
  const todayRange = reportRange(todayKey, todayKey);
  const [orderTotals, todayPayments, services, products, packages] = await adminQuery(Promise.all([
  ...
  const pageCount = Math.max(1, Math.ceil(orderTotals.length / PAGE_SIZE));
  const page = Number.isSafeInteger(requestedPage) && requestedPage > 0 ? Math.min(requestedPage, pageCount) : 1;
  const from = (page - 1) * PAGE_SIZE;
  const ordersResult = await adminQuery(supabase.from("sales_orders").select(ORDER_PROJECTION).eq("clinic_id", member.clinicId).order("created_at", { ascending: false }).order("id", { ascending: false }).range(from, from + PAGE_SIZE - 1));
  ...
  let selected = orders.find((order) => order.id === params.order_id) ?? null;
  if (!selected && params.order_id && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(params.order_id)) {
    const result = await adminQuery(supabase.from("sales_orders").select(ORDER_PROJECTION).eq("clinic_id", member.clinicId).eq("id", params.order_id).maybeSingle());
    if (result.error) throw new Error(adminErrorMessage(result.error.message));
    selected = (result.data ?? null) as unknown as SalesOrder | null;
  }
  const outstanding = orderTotals.filter((order) => order.status === "open" || order.status === "partially_paid").reduce((sum, order) => sum + Number(order.total_amount) - Number(order.paid_amount), 0);
  const receivedToday = todayPayments.reduce((sum, payment) => sum + Number(payment.amount), 0);
  ...
  const saleServices = services.map((item) => ({ id: item.id, name: item.name, price: Number(item.price) }));
  const saleProducts = products.map((item) => ({ id: item.id, name: item.name, price: Number(item.retail_price), meta: `庫存 ${item.stock_on_hand}${item.unit}` }));
  const salePackages = packages.map((item) => ({ id: item.id, name: item.name, price: Number(item.price) }));
  ...
  return <div className="admin-page">
    ...
    <div className="admin-workbench-grid-wide checkout-workbench">
      ...
      <section className="admin-section self-start">{!selected ? <div className="px-5 py-16 text-center"><h2 className="font-semibold text-slate-800">選擇一張銷售單</h2><p className="mt-2 text-sm text-slate-500">點左側單號查看明細、調整成交價、加入商品或套票並收款。</p></div> : <><div className="admin-section-header"><div><div className="flex items-center gap-2"><p className="text-xs text-slate-500">{selected.order_no}</p><span className="badge bg-brand-50 text-brand-700">來源：{sourceLabel(selected)}</span></div><h2 className="font-semibold text-slate-900">{one(selected.patients)?.name ?? "未綁定顧客"}</h2></div><span className="badge bg-slate-100 text-slate-600">{STATUS_LABEL[selected.status] ?? selected.status}</span></div><div className="space-y-5 p-4">
        ...
        {canEdit && <AddSalesItemPanel orderId={selected.id} services={saleServices} products={saleProducts} packages={salePackages} canManageProducts={canManageProducts} />}
