...
import { addCatalogSalesItemAction, addCustomSalesItemAction, recordSalesPaymentAction, updateSalesOrderItemAction } from "./actions";
...
interface SalesOrder {
  id: string; order_no: string; status: string; subtotal: number; discount_amount: number; total_amount: number; paid_amount: number; created_at: string;
...
export default async function CheckoutPage({ searchParams }: { searchParams: Promise<{ order_id?: string }> }) {
  ...
  const canEdit = selected?.status !== "paid" && selected?.status !== "void";

  return <div className="admin-page">
    <div className="admin-page-header">
      <div><p className="eyebrow">預約營運</p><h1 className="admin-page-title">結帳中心</h1><p className="admin-page-description">管理服務、活動報名、商品與套票的銷售明細；成交金額可依現場狀況調整。</p></div>
      <div className="flex flex-wrap gap-2"><Link href="/admin/services" className="btn btn-secondary">設定服務售價</Link><Link href="/admin/checkout/new" className="btn btn-primary"><span aria-hidden="true">＋</span>建立銷售單</Link></div>
    </div>
    ...
    <div className="admin-workbench-grid-wide">
      ...
      <section className="admin-section self-start">{!selected ? <div className="px-5 py-16 text-center"><h2 className="font-semibold text-slate-800">選擇一張銷售單</h2><p className="mt-2 text-sm text-slate-500">點左側單號查看明細、調整成交價、加購與收款。</p></div> : <><div className="admin-section-header"><div><p className="text-xs text-slate-500">{selected.order_no}</p><h2 className="font-semibold text-slate-900">{one(selected.patients)?.name ?? "未綁定顧客"}</h2></div><span className="badge bg-slate-100 text-slate-600">{STATUS_LABEL[selected.status] ?? selected.status}</span></div><div className="space-y-5 p-4">
        ...
        <div><div className="mb-2"><h3 className="text-sm font-semibold text-slate-800">銷售明細</h3><p className="mt-1 text-xs text-slate-500">未結清前可調整每個品項的實際成交單價。</p></div><div className="overflow-hidden border border-slate-200"><table className="tbl"><thead><tr><th>品項</th><th>數量</th><th>成交單價</th><th>小計</th></tr></thead><tbody>{selectedItems.length === 0 ? <tr><td colSpan={4} className="py-6 text-center text-slate-400">尚無品項</td></tr> : selectedItems.map((item) => <tr key={item.id}><td><div className="font-medium">{item.name}</div><div className="text-xs text-slate-400">{ITEM_LABEL[item.kind] ?? item.kind}</div></td><td>{Number(item.quantity).toLocaleString("zh-TW")}</td><td>{canEdit ? <form action={updateSalesOrderItemAction} className="sales-price-editor"><input type="hidden" name="order_id" value={selected.id} /><input type="hidden" name="item_id" value={item.id} /><span>NT$</span><input name="unit_price" type="number" min="0" step="1" defaultValue={item.unit_price} aria-label={`${item.name}成交單價`} /><SubmitButton className="admin-inline-action">更新</SubmitButton></form> : money(item.unit_price)}</td><td className="font-semibold">{money(item.line_total)}</td></tr>)}</tbody></table></div></div>
        {canEdit && <><form action={addCatalogSalesItemAction} className="grid gap-2 sm:grid-cols-[1fr_90px_auto]"><input type="hidden" name="order_id" value={selected.id} /><label className="text-sm"><span className="label">加入既有品項</span><select name="catalog_item" className="input" required defaultValue=""><option value="" disabled>服務、商品或套票</option><optgroup label="服務">{(servicesResult.data ?? []).map((item) => <option key={item.id} value={`service:${item.id}`}>{item.name} · {money(item.price)}</option>)}</optgroup><optgroup label="商品">{(inventoryResult.data ?? []).map((item) => <option key={item.id} value={`product:${item.id}`}>{item.name} · {money(item.retail_price)} · 庫存 {item.stock_on_hand}{item.unit}</option>)}</optgroup><optgroup label="套票">{(plansResult.data ?? []).map((item) => <option key={item.id} value={`package:${item.id}`}>{item.name} · {money(item.price)}</option>)}</optgroup></select></label><label className="text-sm"><span className="label">數量</span><input name="quantity" type="number" min="0.01" step="0.01" defaultValue="1" className="input" required /></label><SubmitButton className="btn btn-secondary self-end">加入</SubmitButton></form>
        <form action={addCustomSalesItemAction} className="grid gap-2 sm:grid-cols-[1fr_90px_120px_auto]"><input type="hidden" name="order_id" value={selected.id} /><label className="text-sm"><span className="label">自訂品項</span><input name="name" className="input" maxLength={160} required placeholder="例如：材料費／現場加購" /></label><label className="text-sm"><span className="label">數量</span><input name="quantity" type="number" min="0.01" step="0.01" defaultValue="1" className="input" required /></label><label className="text-sm"><span className="label">單價</span><input name="unit_price" type="number" min="0" defaultValue="0" className="input" required /></label><SubmitButton className="btn btn-secondary self-end">加入</SubmitButton></form></>}
