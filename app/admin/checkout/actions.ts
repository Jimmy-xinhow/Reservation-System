...
export async function createSalesOrderAction(fd: FormData): Promise<void> {
  ...
  const { data: existing } = source.kind === "appointment"
    ...
    : source.kind === "registration"
      ...
      : { data: null };
  const { data, error } = await adminQuery(service.rpc("create_sales_order", args));
  if (error) {
  ...
  if (!existing && source.kind !== "patient" && sourceAmount !== null) {
    const { data: sourceItem, error: itemError } = await adminQuery(service
      ...
      .limit(1)
      .maybeSingle());
    if (itemError) throw new Error(adminErrorMessage(itemError));
...
async function setSalesOrderItemPrice(clinicId: string, orderId: string, itemId: string, unitPrice: number): Promise<void> {
  const service = createServiceClient();
  const [{ data: order, error: orderError }, { data: items, error: itemsError }] = await adminQuery(Promise.all([
  ...
  if (itemsError) throw new Error(adminErrorMessage(itemsError));
  ...
  const { error: updateError } = await adminQuery(service
    .from("sales_order_items")
    .update({ unit_price: unitPrice, line_total: proposedLineTotal })
    .eq("id", itemId)
    .eq("order_id", orderId)
    .eq("clinic_id", clinicId));
  if (updateError) throw new Error(adminErrorMessage(updateError));
  const { error: recalculateError } = await adminQuery(service.rpc("recalculate_sales_order", { p_clinic_id: clinicId, p_order_id: orderId }));
  if (recalculateError) throw new Error(adminErrorMessage(recalculateError));
}
...
export async function addCustomSalesItemAction(fd: FormData): Promise<void> {
  ...
  if (error) throw new Error(error.message.includes("cannot be changed") ? "已結清的銷售單不能再加品項" : adminErrorMessage(error));
  revalidatePath("/admin/checkout");
}
...
export async function recordSalesPaymentAction(fd: FormData): Promise<void> {
  ...
  const { error } = await adminQuery(createServiceClient().rpc("record_sales_payment", {
    p_clinic_id: member.clinicId,
    p_actor_user_id: member.user.id,
    p_order_id: text(fd, "order_id"),
    p_method: method,
