  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        ...
        <h1 className="admin-page-title">課程與活動報名</h1>
        <p className="admin-page-description">建立活動、場次與票種；公開後，顧客可從瀏覽器、LIFF 或嵌入入口完成報名。</p>
        </div>
      </div>

      {privateLink && <div className="border border-amber-200 border-l-[3px] border-l-amber-600 bg-amber-50 p-4 text-sm text-amber-900"><p className="font-medium">私密報名連結已產生</p><p className="mt-1 break-all text-xs leading-5">請只分享給指定對象；重新產生後，舊連結會失效。</p><a href={privateLink} className="btn btn-secondary mt-3" target="_blank" rel="noreferrer">開啟私密報名頁</a></div>}
      ...
      <div className="space-y-5">
        {eventRows.length === 0 ? <div className="admin-section p-8 text-center text-sm text-slate-400">尚未建立活動。</div> : eventRows.map((event) => {
          ...
          return (
            <section key={event.id} data-membership-plan-count={membershipPlanRows.length} className="admin-section overflow-hidden">
              ...
              <div className="grid gap-5 p-5 lg:grid-cols-2">
                <div><h3 className="mb-3 font-medium text-slate-800">場次</h3>{eventSessions.length === 0 ? <p className="text-sm text-slate-400">尚未建立場次。</p> : <div className="divide-y divide-slate-200 border-y border-slate-200">{eventSessions.map((session) => <div key={session.id} className="bg-slate-50 px-3 py-3 text-sm"><div className="font-medium text-slate-800">{session.name}</div><div className="mt-1 text-slate-500">{formatEventDate(session.start_at)}－{formatEventDate(session.end_at)}</div><div className="mt-1 text-xs text-slate-400">{session.venue || "未設定地點"} · 容量 {session.capacity} · {session.waitlist_enabled ? "開放候補" : "不開放候補"}</div></div>)}</div>}
                  {canEdit && <form action={createEventSessionAction} className="mt-4 grid grid-cols-1 gap-3 border-y border-dashed border-slate-300 bg-slate-50 p-3 sm:grid-cols-2"><input type="hidden" name="event_id" value={event.id} /><label className="text-sm"><span className="label">場次名稱</span><input name="name" required className="input" placeholder="例如：上午場" /></label><label className="text-sm"><span className="label">地點（選填）</span><input name="venue" className="input" placeholder="例如：台北教室 A" /></label><label className="text-sm"><span className="label">開始時間</span><input name="start_at" type="datetime-local" required className="input" /></label><label className="text-sm"><span className="label">結束時間</span><input name="end_at" type="datetime-local" required className="input" /></label><label className="text-sm"><span className="label">可報名人數</span><input name="capacity" type="number" min="1" defaultValue="20" required className="input" /></label><label className="flex min-h-11 items-center gap-2 self-end text-sm text-slate-700"><input name="waitlist_enabled" type="checkbox" defaultChecked />額滿後開放候補</label><div className="sm:col-span-2"><SubmitButton className="btn btn-secondary text-xs">新增場次</SubmitButton></div></form>}
                </div>
                <div><h3 className="mb-3 font-medium text-slate-800">票種</h3>{eventTickets.length === 0 ? <p className="text-sm text-slate-400">未建立票種時，報名頁會使用免費預設票種。</p> : <div className="divide-y divide-slate-200 border-y border-slate-200">{eventTickets.map((ticket) => <div key={ticket.id} className="flex items-center justify-between gap-3 bg-slate-50 px-3 py-3 text-sm"><span className="font-medium text-slate-800">{ticket.name}</span><span className="text-right text-slate-500">{formatAmount(ticket.price)} · {ticket.capacity ? `限 ${ticket.capacity} 人` : "依場次容量"}</span></div>)}</div>}
                  {canEdit && <form action={createTicketTypeAction} className="mt-4 grid grid-cols-1 gap-3 border-y border-dashed border-slate-300 bg-slate-50 p-3 sm:grid-cols-3"><input type="hidden" name="event_id" value={event.id} /><label className="text-sm"><span className="label">票種名稱</span><input name="name" required className="input" placeholder="例如：一般票" /></label><label className="text-sm"><span className="label">售價</span><input name="price" type="number" min="0" defaultValue="0" className="input" /></label><label className="text-sm"><span className="label">票種人數上限（選填）</span><input name="capacity" type="number" min="1" className="input" /></label><label className="text-sm"><span className="label">開始販售（選填）</span><input name="sale_start_at" type="datetime-local" className="input" /></label><label className="text-sm"><span className="label">結束販售（選填）</span><input name="sale_end_at" type="datetime-local" className="input" /></label><label className="text-sm"><span className="label">可使用的套票</span><select name="membership_plan_id" className="input"><option value="">不限套票方案</option>{membershipPlanRows.map((plan) => <option key={plan.id} value={plan.id}>{plan.name}（{plan.credits_total} 堂）</option>)}</select></label><div className="sm:col-span-3"><SubmitButton className="btn btn-secondary text-xs">新增票種</SubmitButton></div></form>}
                </div>
              </div>
              <div className="border-t border-slate-100 p-5"><h3 className="mb-2 font-medium text-slate-800">報名表單 · 第 {eventForm?.version ?? 1} 版</h3>{eventFields.length === 0 ? <p className="text-sm text-slate-500">目前只有姓名、電話與 Email 基本欄位。</p> : <div className="mb-3 flex flex-wrap gap-2">{eventFields.map((field) => <span key={field.id} className="badge bg-brand-50 text-brand-700">{field.label}{field.required ? " · 必填" : ""}</span>)}</div>}{canEdit && <form action={addRegistrationFieldAction} className="grid grid-cols-1 gap-3 border-y border-dashed border-slate-300 bg-slate-50 p-3 sm:grid-cols-2 lg:grid-cols-5"><input type="hidden" name="event_id" value={event.id} /><label className="text-sm"><span className="label">系統欄位代號</span><input name="field_key" required className="input" placeholder="例如：dietary_note" /><span className="help-text block">只供系統辨認；顧客不會看到這個代號。</span></label><label className="text-sm"><span className="label">顧客看到的名稱</span><input name="label" required className="input" placeholder="例如：飲食需求" /></label><label className="text-sm"><span className="label">回答方式</span><select name="field_type" className="input"><option value="text">單行文字</option><option value="textarea">多行文字</option><option value="date">日期</option><option value="select">選單</option><option value="checkbox">勾選確認</option></select></label><label className="text-sm"><span className="label">選單內容（選填）</span><input name="options" className="input" placeholder="素食,葷食,無限制" /><span className="help-text block">使用選單時，以逗號分隔每個選項。</span></label><label className="flex min-h-11 items-center gap-2 self-end text-sm text-slate-700"><input name="required" type="checkbox" />設為必填</label><div className="sm:col-span-2 lg:col-span-5"><SubmitButton className="btn btn-secondary text-xs">新增欄位並建立新版本</SubmitButton></div></form>}</div>
            </section>
          );
        })}
      </div>
