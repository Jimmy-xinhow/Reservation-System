...
export function TreatmentRecordForm({ appointments, action }: { appointments: BeautyAppointmentOption[]; action: (fd: FormData) => Promise<void> }) {
  ...
  return (
    <form action={action} className="card space-y-4 p-5">
      <div><h2 className="font-semibold text-slate-900">新增服務過程紀錄</h2><p className="mt-1 text-sm leading-6 text-slate-500">記錄本次服務前觀察、實際執行內容與後續建議；照片只存於私密空間，不會出現在公開形象頁。</p></div>
      ...
      <label className="block"><span className="label">服務紀錄標題</span><input name="treatment_name" className="input" required maxLength={160} placeholder="例如：初次評估、保濕護理或私人課訓練" /></label>
      ...
      <div><span className="label">服務照片（選填）</span><div className="mt-2 flex flex-wrap gap-3">{photos.map((photo) => <div key={photo.path} className="relative h-24 w-24 overflow-hidden rounded-xl border border-slate-200 bg-slate-50">{photo.previewUrl ? <img src={photo.previewUrl} alt="已上傳服務照片" className="h-full w-full object-cover" /> : <span className="flex h-full items-center justify-center text-xs text-slate-400">已上傳</span>}<button type="button" onClick={() => setPhotos((current) => current.filter((item) => item.path !== photo.path))} className="absolute right-1 top-1 flex h-7 w-7 items-center justify-center rounded-full bg-slate-950/70 text-white" aria-label="移除照片">×</button></div>)}<label className="flex h-24 w-24 cursor-pointer items-center justify-center rounded-xl border border-dashed border-slate-300 text-center text-xs leading-5 text-slate-500 hover:border-brand-400 hover:text-brand-700">{uploading ? "上傳中…" : "＋ 上傳照片"}<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" disabled={uploading || !appointmentId || photos.length >= 6} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); event.currentTarget.value = ""; }} /></label></div></div>
      <label className="flex items-start gap-2 text-sm leading-6 text-slate-600"><input type="checkbox" name="photo_consent" className="mt-1.5" required={photos.length > 0} />已確認顧客同意保存本次服務照片；若沒有照片可不勾選。</label>
      {error && <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}
      <SubmitButton className="btn btn-primary w-full" disabled={appointments.length === 0 || uploading}>儲存服務過程紀錄</SubmitButton>
    </form>
  );
}
