    ...
    <Shell>
      ...
      <div className="card space-y-5 p-5">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {(providerRequired || selectedService?.booking_target === "provider_optional") && <label className="text-sm"><span className="label">服務提供者{providerRequired ? " *" : "（可不指定）"}</span><select className="input" value={doctorId} onChange={(event) => setDoctorId(event.target.value)} required={providerRequired}><option value="">{providerRequired ? "請選擇服務提供者" : "不指定，由系統安排"}</option>{config.doctors.map((doctor) => <option key={doctor.id} value={doctor.id}>{doctor.name}{doctor.specialty ? ` · ${doctor.specialty}` : ""}</option>)}</select></label>}
