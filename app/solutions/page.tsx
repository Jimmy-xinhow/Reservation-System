...
import { Callout, FeatureIcon, MarketingShell, ModuleInterface, PageIntro, PageIntroVisual, PhotoBand, ScenarioMatrix, SectionHeading, SignalStrip, type ModuleKind } from "@/components/MarketingLayout";
...
const scenes = [
  ...
  { number: "06", title: "多據點與多品牌服務", situation: "多個品牌或據點需要各自管理資料、成員與對外入口，工作方式又不完全相同。", system: "每個品牌各自管理服務、顧客、報表與通知，入口與資料範圍清楚分開。", result: "各團隊依自己的工作方式營運，不會混用其他品牌資料。" },
] as const;
...
export default function SolutionsPage() {
  return <MarketingShell>
    ...
    <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-24 lg:px-10"><div className="max-w-2xl"><SectionHeading eyebrow="Real operating scenes" title="每個場景，都要回答三個問題。" description="顧客怎麼進來？團隊怎麼處理？完成後如何延續？以下用實際營運語言說明系統會怎麼接。" /></div><div className="mt-10 grid gap-5 lg:grid-cols-2">{scenes.map((scene) => <article key={scene.number} className="rounded-2xl border border-[#ddd7ca] bg-white p-5 shadow-[0_8px_30px_rgba(31,69,80,.05)] sm:p-6"><div className="flex items-start justify-between gap-4"><span className="text-sm font-mono font-semibold text-[#b08116]">{scene.number}</span><span className="rounded-full bg-[#edf2ef] px-3 py-1 text-xs font-medium text-[#1f4550]">場景說明</span></div><h2 className="mt-7 text-xl font-bold text-[#193b43]">{scene.title}</h2><div className="mt-5 grid gap-4 border-t border-[#eee9df] pt-5 text-sm leading-6"><SceneLine label="現場情況" text={scene.situation} /><SceneLine label="系統怎麼接" text={scene.system} /><SceneLine label="帶來的結果" text={scene.result} accent /></div><ModuleInterface kind={sceneKind(scene.number)} compact /></article>)}</div></section>

    <section className="bg-[#eef3ef] px-5 py-16 sm:px-8 sm:py-20 lg:px-10"><div className="mx-auto max-w-7xl"><div className="grid gap-8 lg:grid-cols-[.7fr_1.3fr] lg:items-end lg:gap-16"><div><p className="eyebrow !text-[#b08116]">One core, many operating shapes</p><h2 className="text-[clamp(1.75rem,3.2vw,2.75rem)] font-bold leading-[1.12] tracking-[-0.04em] text-[#193b43]">同一套核心對應不同的工作方式</h2><p className="mt-4 text-base leading-7 text-[#5d6d6b]">視覺化看出差異：入口、安排與完成方式可以不同，但資料仍然回到同一個營運系統。</p></div><SignalStrip items={[{ label: "一對一", value: "服務時段", detail: "人員／資源" }, { label: "課程活動", value: "場次票種", detail: "容量／報到" }, { label: "場地設備", value: "資源排程", detail: "避免重複預約" }, { label: "多品牌", value: "資料隔離", detail: "品牌資料範圍" }]} /></div><div className="mt-10"><ScenarioMatrix /></div></div></section>

    <PhotoBand src="/marketing/solutions-event-checkin.png" alt="活動現場的報到與入場作業" eyebrow="不同產業共用一套邏輯" title="場景不同，工作畫面仍然清楚" description="一對一服務看時段，活動看場次與報到，場地設備看資源衝突；系統用同一套可配置邏輯接住不同現場。" />

    <section className="bg-[#1f4550] px-5 py-20 text-white sm:px-8 sm:py-24 lg:px-10"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:gap-20"><div><p className="eyebrow !text-[#e2b644]">Configuration over assumptions</p><h2 className="text-[clamp(1.75rem,3.2vw,2.75rem)] font-bold leading-[1.12] tracking-[-0.04em]">同一個核心，依品牌設定成不同流程。</h2><p className="mt-4 max-w-xl text-base leading-7 text-[#c9dcda]">時間制／場次制、指定服務提供者／資源型服務、首次／再次服務、訂金與顧客入口，都以設定決定行為。</p></div><div className="grid gap-3 sm:grid-cols-2"><ConfigPill title="預約模式" value="時間制 ↔ 場次制" /><ConfigPill title="服務目標" value="人員／場地／設備" /><ConfigPill title="顧客入口" value="LINE／瀏覽器／嵌入" /><ConfigPill title="資料範圍" value="每個品牌各自管理" /></div></div></section>
