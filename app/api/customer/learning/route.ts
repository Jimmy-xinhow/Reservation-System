...
export async function POST(request:NextRequest){
  const rate=await checkRateLimit(request,"customer:learning",30);if(!rate.allowed)return fail("操作太頻繁，請稍後再試", rate.unavailable ? 503 : 429);
  try{
    ...
    if(body?.action){
      ...
      if(body.action==="complete"||body.action==="uncomplete"){
        ...
        const {data:existing,error:existingError}=await service.from("course_assessment_submissions").select("status").eq("clinic_id",clinicId).eq("registration_id",registration.id).eq("unit_id",unit.id).maybeSingle();
