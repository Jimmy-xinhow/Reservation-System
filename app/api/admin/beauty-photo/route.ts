...
export async function POST(request: NextRequest) {
  ...
  const sourceKind = String(form?.get("source_kind") ?? "").trim();
  const sourceId = String(form?.get("source_id") ?? "").trim();
  if (!(file instanceof File) || !sourceId || !["appointment", "registration"].includes(sourceKind)) return Response.json({ ok: false, error: "請先選擇服務／課程來源與圖片" }, { status: 400 });
  ...
  const sourceResult = sourceKind === "appointment"
    ? await service.from("appointments").select("id").eq("id", sourceId).eq("clinic_id", clinicId).maybeSingle()
    : await service.from("registrations").select("id").eq("id", sourceId).eq("clinic_id", clinicId).maybeSingle();
  if (!sourceResult.data) return Response.json({ ok: false, error: "服務／課程來源不屬於目前品牌" }, { status: 403 });
  ...
  const path = `${clinicId}/${sourceId}/${randomBytes(16).toString("hex")}.${ext}`;
