...
export async function POST(request: NextRequest) {
  const limited = await rateLimitResponse(request, "membership:portal", 8);
  ...
  try {
    ...
    if (body?.browser_token && !identity) return fail("顧客身分已失效，請重新驗證", 401);
    if (identity) {
      ...
      if (error) return rpcFailure(error, "patient");
    ...
    const plansWithPrices = await Promise.all((plans ?? []).map(async (plan) => {
      ...
      if (typeof price !== "number" || !Number.isInteger(price) || price < 0) {
        throw new Error("會員方案價格資料不完整，請稍後再試");
      }
      return { ...plan, price };
    }));
