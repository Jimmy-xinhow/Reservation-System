...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: order } = await svc.from("payment_orders").select("registration_id, appointment_id").eq("merchant_order_no", merchantOrderNo).eq("provider", "newebpay").maybeSingle();
