...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    const requestedLiff = query.get("liff") === "1";
    ...
    setLiffRequested(requestedLiff);
    const load = async () => {
      try {
        if (requestedLiff) {
          const entryParams = new URLSearchParams();
          if (requestedClinicSlug) entryParams.set("clinic_slug", requestedClinicSlug);
          else if (requestedClinicId) entryParams.set("clinic_id", requestedClinicId);
          const entrySuffix = entryParams.toString() ? `?${entryParams.toString()}` : "";
          const entry = await readApi<{ liff_id: string | null }>(`/api/customer/entry-config${entrySuffix}`);
          setLiffId(entry.liff_id);
        }
        if (eventId) {
  ...
  return <RegistrationForm event={event} clinicSlug={clinicSlug} clinicId={clinicId} accessToken={accessToken} liffRequested={liffRequested} liffReady={liffReady} idToken={idToken} liffError={liffError} onSuccess={setResult} />;
}

function RegistrationForm({ event, clinicSlug, clinicId, accessToken, liffRequested, liffReady, idToken, liffError, onSuccess }: { event: PublicEvent; clinicSlug: string | null; clinicId: string | null; accessToken: string | null; liffRequested: boolean; liffReady: boolean; idToken: string | null; liffError: string | null; onSuccess: (result: RegistrationResult) => void }) {
...
function SuccessCard({ result, clinicSlug, clinicId, accessToken, liffRequested }: { result: RegistrationResult; clinicSlug: string | null; clinicId: string | null; accessToken: string | null; liffRequested: boolean }) {
  ...
  if (accessToken) scope.set("access_token", accessToken);
  if (liffRequested) scope.set("liff", "1");
  const backHref = `/register${scope.toString() ? `?${scope.toString()}` : ""}`;
  const myScope = new URLSearchParams();
  if (clinicSlug) myScope.set("clinic_slug", clinicSlug);
