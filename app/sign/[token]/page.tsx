...
export default async function SignDocumentPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { data, error } = await adminQuery(Promise.resolve().then(() => createServiceClient().from("customer_document_requests").select("status,content_snapshot,template_version,expires_at,signer_name,signed_at,clinics(name),document_templates(name,kind)").eq("token_hash", hash).maybeSingle()));
  ...
  // A redirect/query flag is not proof of a persisted signature.
  const completed = data?.status === "signed";
