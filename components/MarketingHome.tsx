  ...
  return (
    <main className="overflow-hidden bg-[#f7fafd] text-slate-900">
      ...
      <section id="capabilities" className="bg-[#eef5fa] px-5 py-20 sm:px-8 sm:py-24 lg:px-10">
        <div className="mx-auto max-w-7xl"><div className="max-w-2xl"><p className="eyebrow">Product capabilities</p><h2 className="text-3xl font-bold leading-tight tracking-tight text-slate-950 sm:text-4xl">從開門迎客，到看懂營運。</h2><p className="mt-4 text-base leading-7 text-slate-600">標準功能以完整營運為設計基準，依品牌設定切換流程，不把你的產業硬套成單一模板。</p></div><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{capabilities.map((item) => <article key={item.number} className="group min-h-56 rounded-2xl border border-white/80 bg-white p-5 shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-md"><div className="flex items-start justify-between gap-3"><span className={`inline-flex h-9 w-9 items-center justify-center rounded-xl text-xs font-bold ${item.accent}`}>{item.number}</span><span className="text-2xl text-slate-200 transition group-hover:text-[#1b6fc4]" aria-hidden="true">↗</span></div><h3 className="mt-7 font-bold text-slate-900">{item.title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{item.description}</p></article>)}</div></div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-24 lg:px-10"><div className="grid gap-8 lg:grid-cols-2 lg:items-start"><div><p className="eyebrow">A clear commercial boundary</p><h2 className="text-3xl font-bold leading-tight tracking-tight text-slate-950 sm:text-4xl">標準功能完整開放，額外需求清楚分開。</h2><p className="mt-4 max-w-xl text-base leading-7 text-slate-600">基本產品以完整預約與報名營運為核心；清單外的特殊串接或客製模組，另行評估、報價與確認，維護合約不會默默變成新功能開發。</p></div><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-center justify-between border-b border-slate-100 pb-4"><div><p className="text-sm font-bold text-slate-900">另行報價與客製項目</p><p className="mt-1 text-xs text-slate-500">依合作範圍確認，不影響標準功能使用</p></div><span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-800">7 項</span></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{addOns.map((item) => <div key={item} className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2.5 text-sm text-slate-600"><span className="text-[#b8862b]" aria-hidden="true">+</span>{item}</div>)}</div></div></div></section>
