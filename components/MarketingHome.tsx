...
import { ArrowLink, Callout, CapabilityCard, DashboardMockup, MarketingPhoto, MarketingShell, PlatformPreview, SectionHeading, SignalStrip, WorkflowRail } from "@/components/MarketingLayout";
...
export function MarketingHome() {
  return <MarketingShell>
    ...
    <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-10"><SignalStrip items={[{ label: "入口", value: "LINE → LIFF", detail: "顧客從熟悉的入口開始" }, { label: "安排", value: "時間／場次", detail: "依服務目標切換模式" }, { label: "延續", value: "CRM Lite", detail: "完成後留下可用脈絡" }, { label: "治理", value: "多品牌隔離", detail: "平台與品牌各自負責" }]} /></section>
    ...
    <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-24 lg:px-10"><div className="grid gap-10 lg:grid-cols-[.72fr_1.28fr] lg:items-center lg:gap-16"><div><p className="eyebrow !text-[#b08116]">See the work, not just the feature list</p><h2 className="text-3xl font-bold leading-tight tracking-tight text-[#193b43] sm:text-4xl">把每天要處理的事情，放進同一個工作畫面。</h2><p className="mt-4 max-w-xl text-base leading-7 text-[#5d6d6b]">從今天的預約、報名與待跟進，到通知和回訪，管理者先看狀態，再決定下一個動作。</p></div><DashboardMockup variant="operations" /></div><div className="mt-10"><WorkflowRail steps={[{ label: "顧客入口", title: "看見與進入", detail: "LINE Rich Menu、LIFF、瀏覽器" }, { label: "服務作業", title: "預約或報名", detail: "時段、場次、付款、報到" }, { label: "團隊處理", title: "提醒與交接", detail: "狀態、通知、例外日期" }, { label: "後續經營", title: "回訪與分析", detail: "CRM Lite、分眾、報表" }]} /></div></section>
