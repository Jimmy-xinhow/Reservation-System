...
import { ArrowLink, BrandPlate, Callout, CapabilityCard, MarketingShell, SectionHeading } from "@/components/MarketingLayout";
...
export function MarketingHome() {
  return <MarketingShell>
    <section className="relative overflow-hidden bg-[#1f4550] px-5 py-16 text-white sm:px-8 sm:py-24 lg:px-10"><div className="absolute inset-0 opacity-50" style={{ backgroundImage: "linear-gradient(rgba(226,182,68,.09) 1px, transparent 1px), linear-gradient(90deg, rgba(226,182,68,.09) 1px, transparent 1px)", backgroundSize: "54px 54px" }} /><div className="absolute -right-24 top-12 h-80 w-80 rounded-full bg-[#e2b644]/10 blur-3xl" /><div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.08fr_.92fr] lg:items-center lg:gap-20"><div><p className="eyebrow !text-[#e2b644]">XINHOW SERVICE PLATFORM</p><h1 className="max-w-3xl text-4xl font-bold leading-[1.1] tracking-[-0.04em] sm:text-5xl lg:text-6xl">把預約、報名與後續經營，<span className="text-[#e2b644]">放回同一套系統。</span></h1><p className="mt-6 max-w-2xl text-base leading-8 text-[#c9dcda] sm:text-lg">星昊科技為服務型品牌打造多品牌預約與報名 SaaS，從顧客入口、團隊作業到平台營運，讓每個流程都能被看見、被交接、被持續優化。</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/product" className="btn min-h-12 w-fit bg-[#e2b644] px-5 font-semibold text-[#193b43] hover:bg-[#f1ca5b]">看完整產品能力 <span aria-hidden="true">↗</span></Link><Link href="/contact" className="btn min-h-12 w-fit border border-white/20 bg-white/5 px-5 text-white hover:bg-white/10">預約導入諮詢</Link></div><div className="mt-9 flex flex-wrap gap-x-5 gap-y-2 text-xs text-[#a9c2be]"><span>70 項標準功能</span><span>多品牌資料隔離</span><span>LINE → LIFF 優先</span></div></div><div><BrandPlate caption="服務流程的營運基礎" /><div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs text-[#c9dcda]"><div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-[#e2b644]">01</p><p className="mt-1">顧客入口</p></div><div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-[#e2b644]">02</p><p className="mt-1">品牌後台</p></div><div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-[#e2b644]">03</p><p className="mt-1">平台營運</p></div></div></div></div></section>
    ...
    <section className="mx-auto grid max-w-7xl gap-10 px-5 py-20 sm:px-8 sm:py-24 lg:grid-cols-[1fr_1fr] lg:items-center lg:gap-20 lg:px-10"><div><p className="eyebrow !text-[#b08116]">Built around your scene</p><h2 className="text-3xl font-bold leading-tight tracking-tight text-[#193b43] sm:text-4xl">不同產業，不必共用同一種操作方式。</h2><p className="mt-4 max-w-xl text-base leading-7 text-[#5d6d6b]">從一對一服務、團體課程到場地與設備預約，系統以服務目標與資源配置為核心，讓品牌用自己的語言營運。</p><div className="mt-7"><ArrowLink href="/solutions">查看產業場景</ArrowLink></div></div><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-[#1f4550] p-5 text-white"><p className="text-xs uppercase tracking-[0.18em] text-[#e2b644]">Service</p><p className="mt-8 text-lg font-bold">指定人員／資源／場地</p><p className="mt-2 text-sm leading-6 text-[#c4d8d5]">服務目標可依品牌設定，不虛構不需要的角色。</p></div><div className="rounded-2xl border border-[#ddd7ca] bg-[#f7f5ef] p-5"><p className="text-xs uppercase tracking-[0.18em] text-[#b08116]">Growth</p><p className="mt-8 text-lg font-bold text-[#193b43]">完成服務後的回訪</p><p className="mt-2 text-sm leading-6 text-[#6d7b76]">CRM Lite 分眾、互動時間軸與規則式自動化接續經營。</p></div></div></section>
