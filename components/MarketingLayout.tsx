...
function PreviewCrm({ accent, marketing }: { accent: string; marketing: boolean }) {
  return <div className="mt-3 grid gap-3 sm:grid-cols-[.82fr_1.18fr]"><PreviewPanel title={marketing ? "自動化規則" : "顧客清單"}><PreviewList rows={marketing ? [["完成服務後", "LINE 提醒"], ["一段時間未回訪", "Email 關懷"], ["活動報名完成", "加入分眾"]] : [["顧客資料", "依權限查看"], ["互動時間軸", "預約／報名紀錄"], ["分眾標籤", "下一個動作"]]} accent={accent} /></PreviewPanel><PreviewPanel title={marketing ? "投遞狀態" : "互動時間軸"}><div className="space-y-2.5 text-[9px] text-[#536b7a]">{(marketing ? ["待執行 · LINE", "已去重 · Email", "已同意行銷通知"] : ["完成服務 · 已記錄", "通知投遞 · 已記錄", "下一步 · 待處理"]).map((item, index) => <div key={item} className="flex items-center gap-2 border-b border-[#e8eef2] pb-2 last:border-0"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: index === 2 ? accent : "#b8cbd2" }} /><span>{item}</span><span className="ml-auto text-[#8b9ba4]">{index === 2 ? "查看" : "已記錄"}</span></div>)}</div></PreviewPanel></div>;
}
