...
export function PageIntro({ eyebrow, title, description, children, visual, dark = false }: { eyebrow: string; title: string; description: string; children?: React.ReactNode; visual?: React.ReactNode; dark?: boolean }) {
  return <section className={`relative overflow-hidden border-b ${dark ? "border-[#35616a] bg-[#1f4550] text-white" : "border-[#d8d2c5] bg-[#f4f1ea]"}`}><div className="absolute right-0 top-0 h-full w-1/3 border-l border-white/10 opacity-30" aria-hidden="true" /><div className="relative mx-auto grid max-w-7xl gap-12 px-5 py-16 sm:px-8 sm:py-24 lg:grid-cols-[.92fr_1.08fr] lg:items-center lg:gap-20 lg:px-10"><div className="max-w-2xl"><p className={`eyebrow ${dark ? "!text-[#e2b644]" : "!text-[#b08116]"}`}>{eyebrow}</p><h1 className={`max-w-2xl text-[clamp(2.5rem,6vw,5.3rem)] font-bold leading-[1.02] tracking-[-0.055em] ${dark ? "text-white" : "text-[#193b43]"}`}>{title}</h1><p className={`mt-7 max-w-xl text-base leading-8 sm:text-lg ${dark ? "text-[#c4d8d5]" : "text-[#5d6d6b]"}`}>{description}</p>{children && <div className="mt-8 flex flex-wrap gap-3">{children}</div>}</div><div>{visual ?? <div className={`border-l-2 pl-6 text-sm leading-7 ${dark ? "border-[#e2b644] text-[#c4d8d5]" : "border-[#b08116] text-[#6d7b76]"}`}>把複雜的服務流程拆成清楚的入口、工作層級與資料邏輯，讓團隊能真正照著系統營運。</div>}</div></div></section>;
}
...
export function SectionHeading({ eyebrow, title, description, align = "left" }: { eyebrow: string; title: string; description?: string; align?: "left" | "center" }) {
  return <div className={`${align === "center" ? "mx-auto max-w-2xl text-center" : "max-w-2xl"}`}><div className={`mb-4 flex items-center gap-3 ${align === "center" ? "justify-center" : ""}`} aria-hidden="true"><span className="h-px w-10 bg-[#b08116]" /><span className="h-2 w-2 bg-[#e2b644]" /></div><p className="eyebrow !mb-0 !text-[#b08116]">{eyebrow}</p><h2 className="mt-3 text-[clamp(2rem,4vw,3.35rem)] font-bold leading-[1.08] tracking-[-0.045em] text-[#193b43]">{title}</h2>{description && <p className="mt-5 text-base leading-7 text-[#5d6d6b]">{description}</p>}</div>;
}
...
export function Callout({ title, description, href = "/contact", label = "開始規劃" }: { title: string; description: string; href?: string; label?: string }) {
  return <section className="border-y border-[#b8c9c0] bg-[#dfece7] px-5 py-16 sm:px-8 sm:py-20 lg:px-10"><div className="mx-auto flex max-w-7xl flex-col gap-8 lg:flex-row lg:items-end lg:justify-between"><div><p className="eyebrow !text-[#b08116]">Next step</p><h2 className="max-w-2xl text-[clamp(2rem,4vw,3.35rem)] font-bold leading-[1.08] tracking-[-0.045em] text-[#193b43]">{title}</h2><p className="mt-5 max-w-2xl text-base leading-7 text-[#5d6d6b]">{description}</p></div><Link href={href} className="btn min-h-12 w-fit bg-[#1f4550] px-5 text-white hover:bg-[#193b43]">{label} <span aria-hidden="true">↗</span></Link></div></section>;
}
...
export function PageIntroVisual({ variant, dark = false, photoSrc, photoAlt, photoCaption }: { variant: IntroVisualVariant; dark?: boolean; photoSrc?: string; photoAlt?: string; photoCaption?: string }) {
  ...
  if (variant === "product") return <div className={`border shadow-[0_20px_50px_rgba(22,55,62,.18)] ${shell}`}>{photo}<div className="grid gap-0 sm:grid-cols-2"><div className="bg-[#193b43] p-5 text-white"><p className="eyebrow !text-[#e2b644]">平台層</p><h3 className="mt-3 text-xl font-bold">系統擁有者</h3><p className="mt-3 text-sm leading-6 text-[#c9dcda]">品牌租戶、開通交付、健康度與跨品牌治理。</p></div><div className="bg-[#fbfaf6] p-5 text-[#193b43]"><p className="eyebrow !text-[#b08116]">品牌層</p><h3 className="mt-3 text-xl font-bold">品牌營運團隊</h3><p className="mt-3 text-sm leading-6 text-[#6d7b76]">服務、排程、顧客、通知與自己的報表。</p></div></div><p className="border-t border-[#e9e4da] px-5 py-3 text-xs font-semibold text-[#5d6d6b]">兩個管理層，共用同一套資料隔離與權限邏輯</p></div>;
  ...
  return <div className={`border shadow-[0_20px_50px_rgba(22,55,62,.12)] ${shell}`}>{photo}<div className="p-5"><div className="flex items-end justify-between border-b border-[#e9e4da] pb-4"><div><p className="eyebrow !text-[#b08116]">導入路徑</p><h3 className="mt-2 text-xl font-bold text-[#193b43]">從現況到可驗收</h3></div><span className="font-mono text-xs text-[#7a8782]">4 steps</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><OnboardingStep number="01" title="說明現況" detail="入口、排程、收款與通知" done /><OnboardingStep number="02" title="拆解角色" detail="平台、品牌、團隊與顧客" done /><OnboardingStep number="03" title="排列優先" detail="標準流程、加購與客製" active /><OnboardingStep number="04" title="準備驗收" detail="場景、權限、隔離與結果" /></div></div></div>;
}

export function PlatformPreview() {
  const nav = ["平台總覽", "品牌租戶", "營運健康", "跨品牌報表", "平台稽核"];
  return <div className="relative mx-auto w-full max-w-xl lg:ml-auto"><div className="border border-[#7da19c] bg-[#f4f1ea] p-2 shadow-[0_22px_60px_rgba(5,31,38,.3)] sm:p-3"><div className="flex items-center justify-between border-b border-[#d8d2c5] bg-white px-4 py-3"><div><p className="text-[11px] font-bold tracking-[.08em] text-[#193b43]">XINHOW PLATFORM</p><p className="mt-0.5 text-[10px] text-[#7a8782]">系統擁有者控制台</p></div><span className="font-mono text-[10px] text-[#b08116]">/admin/platform</span></div><div className="grid min-h-[300px] grid-cols-[8.4rem_1fr]"><aside className="bg-[#193b43] p-3 text-[10px] text-[#c4d8d5]"><p className="mb-3 text-[9px] font-semibold uppercase tracking-[.16em] text-[#e2b644]">Platform</p>{nav.map((item, index) => <p key={item} className={`px-2 py-2.5 ${index === 0 ? "bg-white/10 font-semibold text-white" : ""}`}>{item}</p>)}<div className="mt-5 border-t border-white/10 pt-3 text-[#7da19c]"><p className="px-2 py-2">品牌後台</p><p className="px-2 py-2">預約與報名</p><p className="px-2 py-2">CRM Lite</p></div></aside><div className="bg-[#fbfaf6] p-4 sm:p-5"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#b08116]">Platform overview</p><h3 className="mt-2 text-xl font-bold text-[#193b43]">平台營運總覽</h3><div className="mt-5 grid gap-2 sm:grid-cols-2"><PreviewRow label="品牌租戶" text="建立、開通、停用" /><PreviewRow label="平台健康" text="通知、金流、部署" /><PreviewRow label="跨品牌報表" text="使用量與活躍度" /><PreviewRow label="平台稽核" text="權限與狀態異動" /></div><div className="mt-5 border-t border-[#d8d2c5] pt-4"><p className="text-[10px] font-semibold text-[#193b43]">品牌開通路徑</p><div className="mt-3 flex items-center gap-2 text-[10px] text-[#6d7b76]"><span className="border border-[#b08116] bg-[#fbf1d9] px-2 py-2">建立品牌</span><span className="text-[#b08116]">→</span><span className="border border-[#cddbd5] bg-[#e8f0ec] px-2 py-2">完成設定</span><span className="text-[#b08116]">→</span><span className="border border-[#cddbd5] bg-[#e8f0ec] px-2 py-2">開始營運</span></div></div></div></div></div><p className="mt-4 text-center text-xs text-[#a9c2be]">畫面依實際平台層級與功能名稱整理，非營運數據展示。</p></div>;
}

function PreviewRow({ label, text }: { label: string; text: string }) {
  return <div className="border-l-2 border-[#b08116] bg-white px-3 py-3"><p className="text-xs font-bold text-[#193b43]">{label}</p><p className="mt-1 text-[10px] leading-5 text-[#7a8782]">{text}</p></div>;
}
