...
- 不把所有章節都做成同一種巨型宋體宣言；主視覺可使用編輯字體，功能型章節須回到容易掃讀的黑體層級。
- 桌面章節標題原則上控制在 2–3 行，欄寬要先配合文案長度；禁止讓完整詞組被拆成「課／程」、「變／化」等單字殘行。
- 前台不得把每個區塊做成帶序號的後台清單；導覽、服務、課程與品牌原則不顯示 `01／02／03`。真正的流程改用「選擇、報名、開始」等自然語意表示先後。
- 服務與課程可以使用編輯式橫列，但不出現「序、類型、內容、操作」等資料表表頭；品牌頁先呈現名稱與價值，再提供清楚動作。
...
- 主視覺標題：`Chiron Sung HK Variable`，500–560；課程索引、流程與 FAQ 等功能型標題改用 `Chiron Hei HK Variable`，避免每段都像同一張宣言海報。
