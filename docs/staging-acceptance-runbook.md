...
2. 先完成可還原備份，再依 README「既有資料庫 migration 順序」完整執行至 `202609070003_membership_card_and_redemption.sql`；其中 `migration_marketing_opt_in_sync.sql`、品牌頁、共用 API 限流、平台聚合報表、checkout lint、報名付款同步、出勤與套票跨場景兌換 migration 都不可跳過。
...
- 單一 LIFF：先開啟 `home` 服務首頁，再逐一開啟 `booking`、`appointments`、`events`、`tickets`、`membership`、`support`、`brand`；停用模組不得出現在首頁，同一 LINE 綁定多人時須可切換，且不得讀到另一品牌顧客。
