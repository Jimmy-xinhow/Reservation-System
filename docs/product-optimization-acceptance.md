...
| P1 | 顧客入口先顯示服務首頁，依品牌設定提供預約、紀錄、活動、票券、會員、客服與品牌捷徑 | `app/book/CustomerEntry.tsx`、`lib/customer-entry.ts` |
| P1 | 後台與顧客介面統一字級、操作高度、鍵盤焦點、白話狀態與可展開技術資訊 | `app/globals.css`、`components/TechnicalDetails.tsx`、`lib/admin-display.ts` |
| P0 | 登入、Rich Menu 與非預期 API 錯誤不直接顯示第三方或資料庫原始訊息，保留錯誤識別碼供維護查找 | `app/admin/login/page.tsx`、`app/admin/error.tsx`、`app/admin/line-actions.ts`、`lib/http.ts` |
...
- 品牌管理者：管理品牌資料、人員權限、服務、預約、報名、CRM Lite、報表與通訊設定。
- 品牌員工：依品牌管理者授予的品牌管理、營運操作或指定服務提供者權限工作；不得管理品牌人員。
- 系統管理者：管理品牌帳號、系統人員權限、標準功能服務狀態與加購合作備註，不直接取代品牌人員的日常操作。
- 系統員工：只使用系統管理者逐項授予的跨品牌工作權限；不得管理系統人員。
...
- 完成手機 375／414px、平板 768／1024px 與桌面 1440px 的公開入口、顧客紀錄、後台導覽與 CRM 操作檢查。
