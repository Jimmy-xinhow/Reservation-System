...
## 2026-09-06 正式環境三產業 DEMO

| 項目 | 正式環境實際操作證據 | 結果 |
|---|---|---|
| 正式資料庫升級 | Pro 專案 migration 對齊至 `202609060001`；`public` schema DB lint | PASS；舊資料庫的 `clinic_members` 角色檢查已接受現行 `owner` 角色，lint 零警告 |
| 美業 DEMO | ÉLAN 品牌頁、3 項護理、2 位服務人員、客製問答、加購、套票、優惠碼、療程紀錄、庫存／採購／獎金、CRM，以及換季保養公開講座 | 正式資料與公開 UI PASS；護膚首圖、講座票種、必要問答及條款均已實際載入 |
| 線上課程 DEMO | OPENROOM 品牌頁、付費班／免費公開課、選課諮詢、票種、優惠碼、套票、5 個學習單元、測驗、作業、完課條件與既有示範報名 | 正式資料與公開 UI PASS；付費提示、兩種票價及課程問答均已實際載入 |
| 皮拉提斯／瑜珈 DEMO | FORME 品牌頁、4 項私人服務、2 個團課、教室／器械資源、加購、套票、月訂閱、凍結排程及既有示範報名 | 正式資料與公開 UI PASS；私人課客製欄位、團課容量、付費提示及安全條款均已實際載入 |
| 多服務人員預設選擇 | 一個品牌有多位服務人員時，預約頁不再任意預選第一位；必選時顯示「請選擇服務提供者」與必填標示，單一人員品牌仍可自動帶入 | 正式 UI 複驗 PASS；不再出現服務與人員被任意配對 |
| 外部渠道隔離 | 三品牌 `LINE`、`Email` 與正式金流憑證均保持未啟用，DEMO 顧客使用 `example.com` 且未勾行銷同意 | PASS；等待使用者最後依渠道逐項補上正式帳號與測試商店 |
...
1. 新環境執行 `supabase/schema.sql`；既有環境依 README 的 49 支 migration 順序執行。
