...
begin
  ...
  if v_taken >= s.capacity or (p_ticket_type_id is not null and ticket.capacity is not null and v_ticket_taken >= ticket.capacity) then
    ...
    if v_membership_code is not null then
      ...
      v_membership_id := membership_row.id;
      v_amount := 0;
      ...
      if v_code is not null and v_amount > 0 then
        ...
        v_discount_code_id := discount_row.id;
  ...
  insert into registrations (
    ...
    v_discount_code_id, v_discount_amount,
    v_membership_id,
  ...
  if v_membership_applied then
    perform consume_membership_credit(p_clinic_id, membership_row.id, 'registration', 'registration', v_id, membership_row.plan_service_id, null, 'registration membership redemption');
