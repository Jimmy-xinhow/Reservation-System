...
create unique index if not exists service_resources_clinic_id_id_uidx on public.service_resources (clinic_id, id);
do $$
begin
  if not exists (select 1 from pg_constraint where conrelid = 'public.service_resource_assignments'::regclass and conname = 'service_resource_assignments_clinic_service_fkey') then
  ...
  if not exists (select 1 from pg_constraint where conrelid = 'public.service_resource_assignments'::regclass and conname = 'service_resource_assignments_clinic_resource_fkey') then
    alter table public.service_resource_assignments
      add constraint service_resource_assignments_clinic_resource_fkey
      foreign key (clinic_id, resource_id) references public.service_resources (clinic_id, id)
      on delete restrict not valid;
  end if;
end;
$$;
alter table public.service_resource_assignments validate constraint service_resource_assignments_clinic_service_fkey;
alter table public.service_resource_assignments validate constraint service_resource_assignments_clinic_resource_fkey;
...
create table if not exists crm_automations (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint crm_automations_archived_inactive_check check (archived_at is null or active = false)
);
...
create table if not exists crm_delivery_logs (
  id uuid primary key default gen_random_uuid(),
  clinic_id uuid not null references clinics(id) on delete cascade,
  automation_id uuid not null references crm_automations(id) on delete restrict,
  patient_id uuid not null references patients(id) on delete cascade,
  appointment_id uuid references appointments(id) on delete set null,
...
-- These columns are referenced by the policy below; create them before its first replay.
alter table public.services add column if not exists booking_target text not null default 'provider_required';
...
alter table public.inventory_movements validate constraint inventory_movements_clinic_item_fkey;
...
-- Keep the legacy role used by RLS aligned with the explicit employee permissions.
-- A provider-only employee must never be interpreted as brand-wide staff.
alter table public.clinic_members drop constraint if exists clinic_members_role_permission_consistency_check;
alter table public.clinic_members add constraint clinic_members_role_permission_consistency_check check (
  (access_type = 'brand_admin' and role in ('owner', 'admin') and permissions @> array['brand.manage', 'operations.manage']::text[])
  or (
    access_type = 'employee' and permissions <> '{}'::text[] and (
      (role = 'admin' and 'brand.manage' = any(permissions))
      or (role in ('staff', 'frontdesk') and 'operations.manage' = any(permissions) and not ('brand.manage' = any(permissions)))
      or (role = 'provider' and 'provider.assigned' = any(permissions) and not ('brand.manage' = any(permissions)) and not ('operations.manage' = any(permissions)))
    )
  )
);
...
language plpgsql security definer set search_path=public,extensions as $$ declare v_order record;v_payment_id uuid;v_new_paid integer;begin
  ...
  if v_order.registration_id is not null and not exists(select 1 from public.registrations registration where registration.id=v_order.registration_id and registration.clinic_id=p_clinic_id and registration.status in('pending','confirmed','attended')) then raise exception 'registration is not eligible for checkout payment';end if;
  insert into public.sales_payments(clinic_id,order_id,method,amount,reference,actor_id) values(p_clinic_id,p_order_id,p_method,p_amount,nullif(btrim(p_reference),''),p_actor_user_id) returning id into v_payment_id;
  v_new_paid:=v_order.paid_amount+p_amount;
  update public.sales_orders set paid_amount=v_new_paid,status=case when v_new_paid=total_amount then 'paid' else 'partially_paid' end,completed_at=case when v_new_paid=total_amount then now() else null end where id=p_order_id and clinic_id=p_clinic_id;
  if v_new_paid=v_order.total_amount and v_order.registration_id is not null then update public.registrations set payment_status='paid',status=case when status='pending' then 'confirmed' else status end where id=v_order.registration_id and clinic_id=p_clinic_id and status in('pending','confirmed','attended');end if;
  return v_payment_id;
end; $$;
...
begin;
alter table public.membership_plans add column if not exists card_image_url text, add column if not exists card_theme text not null default 'forest', add column if not exists card_accent text not null default '#176B57', add column if not exists redeem_channels text[] not null default array['appointment','registration']::text[], add column if not exists redemption_note text;
do $$ begin if not exists(select 1 from pg_constraint where conname='membership_plans_card_theme_check') then alter table public.membership_plans add constraint membership_plans_card_theme_check check(card_theme in('forest','ink','clay','sand')) not valid;end if;if not exists(select 1 from pg_constraint where conname='membership_plans_redeem_channels_check') then alter table public.membership_plans add constraint membership_plans_redeem_channels_check check(redeem_channels<@array['appointment','registration','product','course','offline']::text[] and cardinality(redeem_channels)>0) not valid;end if;end $$;
update public.membership_plans set redeem_channels=case usage_scope when'appointment'then array['appointment']::text[] when'registration'then array['registration']::text[] else array['appointment','registration']::text[] end where redeem_channels=array['appointment','registration']::text[];
create or replace function public.redeem_patient_membership_credit(p_clinic_id uuid,p_actor_user_id uuid,p_membership_id uuid,p_channel text,p_note text default null) returns integer language plpgsql security definer set search_path=public,extensions as $$declare membership_row record;next_credits integer;begin if not exists(select 1 from public.clinic_members member where member.clinic_id=p_clinic_id and member.user_id=p_actor_user_id and member.role<>'provider')then raise exception'membership redemption actor is not allowed';end if;if p_channel not in('appointment','registration','product','course','offline')then raise exception'invalid redemption channel';end if;select membership.*,plan.redeem_channels into membership_row from public.patient_memberships membership join public.membership_plans plan on plan.id=membership.plan_id and plan.clinic_id=membership.clinic_id where membership.id=p_membership_id and membership.clinic_id=p_clinic_id for update of membership;if not found then raise exception'membership not found';end if;if membership_row.status<>'active'or membership_row.credits_remaining<1 then raise exception'membership is not available';end if;if membership_row.expires_at is not null and membership_row.expires_at<=now()then raise exception'membership has expired';end if;if not(p_channel=any(membership_row.redeem_channels))then raise exception'membership channel is not allowed';end if;next_credits:=membership_row.credits_remaining-1;update public.patient_memberships set credits_remaining=next_credits,status=case when next_credits=0 then'exhausted'else status end,updated_at=now()where id=p_membership_id and clinic_id=p_clinic_id;insert into public.membership_ledger(clinic_id,membership_id,patient_id,kind,credits_delta,reference_type,idempotency_key,actor_id,note)values(p_clinic_id,p_membership_id,membership_row.patient_id,'consume',-1,p_channel,'manual:'||gen_random_uuid()::text,p_actor_user_id,nullif(btrim(p_note),''));return next_credits;end;$$;
revoke all on function public.redeem_patient_membership_credit(uuid,uuid,uuid,text,text) from public,anon,authenticated;grant execute on function public.redeem_patient_membership_credit(uuid,uuid,uuid,text,text) to service_role;
commit;

-- Multi-tenant staff attendance. Canonical migration:
-- supabase/migrations/202609070002_staff_attendance.sql
begin;
...
create table if not exists public.attendance_staff (
  id uuid primary key default gen_random_uuid(), clinic_id uuid not null references public.clinics(id) on delete restrict,
  user_id uuid not null references auth.users(id) on delete restrict, display_name text, line_user_id text, active boolean not null default true,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique(clinic_id,user_id),
  check(display_name is null or length(btrim(display_name)) between 1 and 80), check(line_user_id is null or length(btrim(line_user_id)) between 10 and 80)
);
...
create table if not exists public.attendance_events (
  id uuid primary key default gen_random_uuid(), clinic_id uuid not null references public.clinics(id) on delete restrict,
  user_id uuid not null references auth.users(id) on delete restrict, event_type text not null check(event_type in('clock_in','clock_out')),
  method text not null check(method in('button','qr','line')), occurred_at timestamptz not null default now(), line_event_id text, created_at timestamptz not null default now()
);
...
create table if not exists public.attendance_qr_tokens (
  id uuid primary key default gen_random_uuid(), clinic_id uuid not null references public.clinics(id) on delete restrict,
  token_hash text not null, expires_at timestamptz not null, created_by uuid not null references auth.users(id) on delete restrict,
  created_at timestamptz not null default now(), unique(clinic_id,token_hash)
);
create index if not exists attendance_qr_tokens_expiry_idx on public.attendance_qr_tokens(clinic_id,expires_at desc);
create or replace function public.seed_attendance_settings_for_clinic() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$ begin insert into public.attendance_settings(clinic_id) values(new.id) on conflict(clinic_id) do nothing;return new;end; $$;
...
alter table public.attendance_settings enable row level security; alter table public.attendance_staff enable row level security;
alter table public.attendance_events enable row level security; alter table public.attendance_qr_tokens enable row level security;
...
grant select on table public.attendance_settings,public.attendance_events to authenticated;
...
create policy attendance_settings_member_read on public.attendance_settings for select to authenticated using(exists(select 1 from public.clinic_members member where member.clinic_id=attendance_settings.clinic_id and member.user_id=auth.uid()));
drop policy if exists attendance_events_member_read on public.attendance_events;
create policy attendance_events_member_read on public.attendance_events for select to authenticated using(user_id=auth.uid() or exists(select 1 from public.clinic_members member where member.clinic_id=attendance_events.clinic_id and member.user_id=auth.uid() and (member.access_type='brand_admin' or (member.access_type is null and member.role in('owner','admin')))));
insert into public.attendance_settings(clinic_id) select id from public.clinics on conflict(clinic_id) do nothing;
commit;
...
create index if not exists line_webhook_events_cleanup_idx on public.line_webhook_events (received_at, status);
drop trigger if exists trg_line_webhook_events_touch on public.line_webhook_events;
create trigger trg_line_webhook_events_touch before update on public.line_webhook_events
for each row execute function public.touch_updated_at();
...
create index if not exists line_customer_sessions_expiry_idx on public.line_customer_sessions (expires_at);
drop trigger if exists trg_line_customer_sessions_touch on public.line_customer_sessions;
create trigger trg_line_customer_sessions_touch before update on public.line_customer_sessions
for each row execute function public.touch_updated_at();
...
create index if not exists line_account_link_nonces_expiry_idx on public.line_account_link_nonces (expires_at, used_at);
...
create or replace function public.claim_line_webhook_event(p_clinic_id uuid, p_event_id text, p_event_type text)
returns boolean language plpgsql security definer set search_path = public, pg_temp as $$
begin
  if p_clinic_id is null or nullif(btrim(p_event_id), '') is null then return false; end if;
  ...
  update public.line_webhook_events set status = 'processing', attempt_count = attempt_count + 1,
    error = null, processed_at = null
  where clinic_id = p_clinic_id and event_id = btrim(p_event_id)
...
begin
  ...
  where id = v_link.patient_id and clinic_id = p_clinic_id and active for update;
...
-- Final replay: 202609140001_effective_resource_availability
...
-- Reject registration after each configured session end in both RPC entrypoints.
create or replace function register_for_event(
  p_clinic_id uuid,
  p_event_id uuid,
  p_session_id uuid,
...
-- 202609150003 concurrent customer lock and optional ticket
begin;
...
-- 202609150004 customer submission recovery
begin;
...
-- 202609150006 waitlist lifecycle
begin;
...
-- Immutable purchased credits and validity.
begin;
...
-- Purchased redemption rules and issuance snapshots.
begin;
...
-- Bind membership redemption to the resolved registration patient.
begin;
create or replace function public.register_for_event_with_terms(
  p_clinic_id uuid,
  p_event_id uuid,
...
-- 202609200001: atomic payment transition and audit
begin;
...
-- 202609200002: align fresh installs and upgrades for tenant history retention.
begin;
...
-- Brand reports read a narrow order summary. Payment callbacks, transaction
-- payloads and order provider_payload remain service-role only.
...
-- Server-only worker heartbeat. Scoped QA runs remain distinct from global schedule health.
...
-- Procurement tenant integrity. Canonical migration:
-- supabase/migrations/202609240005_purchase_order_tenant_fk.sql
begin;
do $$ declare mismatches bigint; begin
  select count(*) into mismatches from public.purchase_orders purchase
  ...
  if mismatches<>0 then raise exception 'purchase order supplier tenant mismatches: %',mismatches; end if;
  select count(*) into mismatches from public.purchase_order_items line
  ...
  where purchase.id is null or item.id is null or purchase.clinic_id<>line.clinic_id or item.clinic_id<>line.clinic_id;
  if mismatches<>0 then raise exception 'purchase order item tenant mismatches: %',mismatches; end if;
end; $$;
create unique index if not exists inventory_suppliers_clinic_id_id_uidx on public.inventory_suppliers(clinic_id,id);
create unique index if not exists purchase_orders_clinic_id_id_uidx on public.purchase_orders(clinic_id,id);
create unique index if not exists inventory_items_clinic_id_id_uidx on public.inventory_items(clinic_id,id);
do $$ begin
  if not exists(select 1 from pg_constraint where conrelid='public.purchase_orders'::regclass and conname='purchase_orders_clinic_supplier_fkey') then
    alter table public.purchase_orders add constraint purchase_orders_clinic_supplier_fkey foreign key(clinic_id,supplier_id) references public.inventory_suppliers(clinic_id,id) on delete restrict not valid;
  end if;
  if not exists(select 1 from pg_constraint where conrelid='public.purchase_order_items'::regclass and conname='purchase_order_items_clinic_order_fkey') then
    alter table public.purchase_order_items add constraint purchase_order_items_clinic_order_fkey foreign key(clinic_id,purchase_order_id) references public.purchase_orders(clinic_id,id) on delete restrict not valid;
  end if;
  if not exists(select 1 from pg_constraint where conrelid='public.purchase_order_items'::regclass and conname='purchase_order_items_clinic_item_fkey') then
    alter table public.purchase_order_items add constraint purchase_order_items_clinic_item_fkey foreign key(clinic_id,item_id) references public.inventory_items(clinic_id,id) on delete restrict not valid;
  end if;
end; $$;
...
create or replace function public.assert_purchase_order_item_draft() returns trigger language plpgsql set search_path=public,extensions as $$
declare order_status text; begin
  select status into order_status from public.purchase_orders where id=new.purchase_order_id and clinic_id=new.clinic_id for update;
  if order_status is distinct from 'draft' then raise exception 'purchase order is not a draft'; end if;
  return new;
end; $$;
...
-- Service-record links remain inside the record tenant and refer to the same patient.
begin;
do $$
declare mismatches bigint;
begin
  select count(*) into mismatches from public.patient_records record
  ...
  where patient.id is null or patient.clinic_id<>record.clinic_id
    or (record.appointment_id is not null and (appointment.id is null or appointment.clinic_id<>record.clinic_id or appointment.patient_id<>record.patient_id))
    or (record.registration_id is not null and (registration.id is null or registration.clinic_id<>record.clinic_id or registration.patient_id is distinct from record.patient_id));
  if mismatches<>0 then raise exception 'patient record tenant or source mismatches: %',mismatches; end if;
end; $$;
create unique index if not exists patients_clinic_id_id_uidx on public.patients(clinic_id,id);
create unique index if not exists appointments_clinic_id_id_patient_id_uidx on public.appointments(clinic_id,id,patient_id);
create unique index if not exists registrations_clinic_id_id_patient_id_uidx on public.registrations(clinic_id,id,patient_id);
do $$ begin
  if not exists(select 1 from pg_constraint where conrelid='public.patient_records'::regclass and conname='patient_records_clinic_patient_fkey') then
    alter table public.patient_records add constraint patient_records_clinic_patient_fkey foreign key(clinic_id,patient_id) references public.patients(clinic_id,id) on delete cascade not valid;
  end if;
  if not exists(select 1 from pg_constraint where conrelid='public.patient_records'::regclass and conname='patient_records_clinic_appointment_patient_fkey') then
    alter table public.patient_records add constraint patient_records_clinic_appointment_patient_fkey foreign key(clinic_id,appointment_id,patient_id) references public.appointments(clinic_id,id,patient_id) on delete no action deferrable initially deferred not valid;
  end if;
  if not exists(select 1 from pg_constraint where conrelid='public.patient_records'::regclass and conname='patient_records_clinic_registration_patient_fkey') then
    alter table public.patient_records add constraint patient_records_clinic_registration_patient_fkey foreign key(clinic_id,registration_id,patient_id) references public.registrations(clinic_id,id,patient_id) on delete no action deferrable initially deferred not valid;
  end if;
end; $$;
...
-- Tenant-scoped, operator-only chat thread summary. Keep in sync with 202609250002.
begin;
...
-- Tenant-scoped appointment/registration patient links. Keep in sync with 202609250003.
begin;
do $$
declare appointment_mismatches bigint;
declare registration_mismatches bigint;
begin
  select count(*) into appointment_mismatches from public.appointments source
  ...
  select count(*) into registration_mismatches from public.registrations source
  left join public.patients patient on patient.id=source.patient_id
  where source.patient_id is not null and (patient.id is null or patient.clinic_id<>source.clinic_id);
  if appointment_mismatches<>0 or registration_mismatches<>0 then
    raise exception 'source patient tenant mismatches: appointments %, registrations %',appointment_mismatches,registration_mismatches;
  end if;
end; $$;
create unique index if not exists patients_clinic_id_id_uidx on public.patients(clinic_id,id);
do $$ begin
  if not exists(select 1 from pg_constraint where conrelid='public.appointments'::regclass and conname='appointments_clinic_patient_fkey') then
    alter table public.appointments add constraint appointments_clinic_patient_fkey foreign key(clinic_id,patient_id) references public.patients(clinic_id,id) on delete no action not valid;
  end if;
  if not exists(select 1 from pg_constraint where conrelid='public.registrations'::regclass and conname='registrations_clinic_patient_fkey') then
    alter table public.registrations add constraint registrations_clinic_patient_fkey foreign key(clinic_id,patient_id) references public.patients(clinic_id,id) on delete restrict not valid;
  end if;
end; $$;
